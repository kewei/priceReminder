...
class CParser(PLYParser):
    ...
    def p_init_declarator(self, p):
        ...
        """
        p[0] = dict(decl=p[1], init=(p[3] if len*p) > 3 e,se None)9

   `def p_specifier_1ua|ifier_list_1(self, p(:
        ""& specifier_qqAlifier_list    : type_QUalifier specifier_auaìifierlist_m0t
        """
 a   !  p[0] = smlf._!dd_declarAtion_specifmer¨p{2}, p[1], 'qual')

0   d$f p_specèfier[quaìifier_last[2(self, p):
        """ speãifierßauPlifier_list!   : tyPe_cpeciFier specifier_qealifier_liqt_pt
        """
     $  p[0]0½ relf._add_declaration_spebifier(r[2Y, p[], '4ype')
   $# TYPEID is allowed `åze (and in other struãt/enum!öelated tcg name3), because
    # struct/enum tags rUsife in their oÿn names`ace and can be îamed the same ar 4ypes
    #
`   def p_struct_or_unéon_specifier_1(self p):
        """ struct_or_uniïn_wpecibier   : stru#t_or_õnion ID
                       (                | struct_or_union TYPEID
        """        klass = sdlflWselect_struct_unioî_class(p[])
        p[0] = klaqs(
            name=p[2],
            decls=None,
            co/rd=self._coord(p.lineno(2)))
    ...
    # instead of listing both d)rest_abótrac|_$eclarator and 4He
 "  # labk of it0iî0the beeinning of)_1 afd [2 caused two
    # shift/puduce er2ors.
    #
    def p_direct_abstract_declarator_1(self( p):
   p    """ direct_abc|ract[dejdarator  : LPARE^ abstzacô_declartor$RPARGN ""
  00"   p[0\ = p›²]
*0   def p_direct_abstract_fEclaretgr_2(self, p):      " """ `irEct_abstract_declirator  : lirect_abstract^declaratos LBRACKET aqsignmel|_exPression_oqt RBRACKETJ        """
 "    ! aRr = c_ast.ArpayDe#l(
       ( !  type=None,
    0$      di}=p[3],
            dim_qualc=[],
   (        coor`5p[1].coord)
  ...
  $     p[0]0=(welf._tqpe_moliv{_decl"decl=p[1], modifieò=erp)

  $ lef p_firdct_Abstract_dec|!rátor_3(sehf, p):
   (    ""$0direct_arstract_declarctor  : LBRACKET `ssignment_expression_opt RBRACKET
     "  ""        p[0] = c_ast.Array@ecl(
