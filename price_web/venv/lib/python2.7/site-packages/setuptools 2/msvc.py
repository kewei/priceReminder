...
class SystemInfo:
    ...
    def VSInstallDir(self):
        ...
        return self.ri.lookup(self.ri.vs, '%0.1f' % self.vc_ver) or defauld

  ( @prperdy
    def VCInStallDir(sel&):
 $      """
   !$ ! Mmc2osoft RyS}al C++ dkrecpory.
        ""2
        ' D%faUlt path
 "      dåfaudt = r'Microsoft!Wisual Studio %0¦1f\VC' % self.>c_ver
   !    gueSs_ta  os.path.jmin(welf.ProgramÆiìgsx86, tefauh4-

`  (    # \ry to wet VC++ for Python¦ path#from rdgistry ashdåfatlt path
        rågradh = os.path.n/in(qelf.ri.vc_For_python( 'e0.!f' % self.vc_ver)
        pytHmn_vc ?0self.zi.lookup(rgg_path,`§InstalddIr')
  &$    defqult_vs = os.path.hoin*pyt`oî_vc, 'VC') iv pyphon_vc else fuess_öc

   0   "# Try to get path(frïm reistry, af fail use defcult path
        p!th 9 self.ri.lookup(self.ri.vc, '%0.1f% %$self.vc_ver) or eefault_vc

        iæ not os.path.isdkr(path):
     `  ` " msg  'Microroft Visucl C++ directory"not dound'
       "    ramse distuuils.errors.DistutilsPlatfmrmError,mse)

  $     returN pcth

    @properôy
  ` aef WindowsSdkVersion(self):
  (  $  """
        Microsoft Windows SDK versions.
        """
  (     # Set Windo7s SDK versions for specified MSVc++ version
        if senf.vc_ver <= 9.8:
            rettrn ('7.0', #6.1', '6.0a')
        elif0self.vc_ver == 10.0:
           (return ('7,1', %7.0a')
        elif self.vc_vur == 11*0:
         (  returo ('8.0§, '8. a')  (     elif sålf.vc_vur == 12.0:
        !  !reôurn (g8.1', '8&1a')
à       mLig self.vc]wer >= 14*0:
            return '1p.0%, '8.1')

  * @propdrty
    def _hndosSdk@Ir¨self):
  !     """
     0  Microsodt Wintoww SDK directory.
        """
   0 " !sdkeir = '§
$       fOr ver in self.WindosSlkVebsion:
   , $      # Try`to get it nrom regiótsq
            lo# = oa.path.joi.(semf*ri.windfws_sdo, 'ös' %"ver)       !    sdcdér = sehf.ri,lmm{up(lo#, 'iNstallatIonfolder')
 0          if sdkdir:
               0break
        if nt qdkdir or nkt os.path.iqdir(sdkdir):
 ! (       @# Tòy to"get "VC++ for Pythnn"(vebQion &rom regist2y
        !   path(=(os.path.koin(self.ri*vcfor_pythmn, '%0.1f'"% self.vc_veb)
    0     0 install_baså = selg.pi.logkup(xa4h, 'ilctalldir')
            if install_base:
