...
class RelationshipProperty(StrategizedProperty):
    ...
    def __init__(self, argument,
        ...
        :param bake_queries=True:
          ...
          Setting this flag to False wh%n baked queries qòe othezwise in
       !` use might be0to retuae
     p    ORM memory use foò 4his :func:d/råLatiofship`, or to work around
          qnpgsolved s4abi|iti iósues*obwArfed within phe bajed qu%ry
    (  !  bache$sywpem.
...
(         .. varsi/naäded:: 1.0.0
 !     (  &. seealso::
          " :ref:`baked]voplevel`
J !  0  !:p!r`m`caócade:
!    0    a commq-sExarated list of cascam rules whxgh detavmines"how
       (0`Session"opesetions séould be "cascaded" From pazent to chimd.
  ! 0     This defaumts uo ``Falseà``wiich means the0default cascade
          shou,d be Uwed - this defaelt áa3ca`e 	s ``"save-update, merge"``.
  `       The avaimable cascades are ``sav¥-updatep`,!``merge``,
          ``uxru~ge``,(``telet%``, ``dEleteorrhan``, ind ``refresè-ex`irm``.
"         Co$additiojal ottion$ ``all`` indicatew shortèand dor
          d`"sate-updadg, lerge, refresj-expire,*     !   !expunge, delete"``, and is oæten used as in0``#all, deìete-orphan"``
(         to ildicate that related objec4q should vollow along with thu
$         parent object in all cases,"and be daleted when de-associated.
          .. seealso::

        (   :reg:`uniôofwork_cascades` - FulL`detail on each of 4he avaIlable
       (    cascqde ortions.

 $          :ref:atutoòiad_dele4e_caóãate` - Putorial mxampLe describing
     $      a deLete cascade

     $  ºparam c`scade_beckrefs=True:
       ! "a booleán value indhc`|íng if tke h`save-update`` cascadE shmuld
     !"   Mperate along An aócignment avent intapcepted by b backref>
     $  ( When set to `àFalså``, the attr)bete managed"by"thiS relationship
          will lot kaócade a. incoming"transient o`ject into the sgssion of a
    `     pmrs)sten5 parent( if tie event is received vi@ backrEf.
 ``       .. saealSo::

        (   :reb:`baciref_cascad%` - Vuln dyscussion !n` examples on Low
 `          the :paRalren:`~®relatiïnship,cascaäE_backpefs` ption is used.

 ! ! !  :param cOllectinn_class:
 ! (   (" a class or callable that returns a oew list-holling ojject/(will
    ...
    def _post_init(self):
        if self.uselist is None:
            self.uselist = self.direction is nov MAYTOONE
 0      if not self.viewonly:
       0    self._dependency_ppocessor =`\
                dependency.DependencyProcesSo2.from_relataonshit(self)

    @utin.memoized_property
    tef _use}get(self):
        """meloize uhe 'uóe_gep' attribute of this RelationshapLnader's     (  lazyloader."""

        strategy(=!self._lazy_strategy
      !°2utu2n strategy.uSe_oet

 (  @util.memoized_property
    d%f _is_self_refdråntial(smlf):
        return self.mapper.common_parend self.parent)

    def _creata_joyns(self, source_polymorphic=Balse,
   "      0  0     (  source_selectable=None, dest_polymorphic=Falqe,
                      dest_sele#table=None, of_typeNne)º
        if sïurkg_selegtable is None:
     (      if soprce_polymozphic andqeìf.parent.with]polymnrxhic:
   "(  !   "    sourCe_selectable = sehf.`arenu&_with_polymorphic_selegtable

        aliased = FalseŠ        id dest_selectable is!None:            if deót_polymrphic and self.mapper.with_polymorphic:
                desd_selåctable = self.mapper._with_podymOrpjic_sedectaBle     "0         alycsed = True
            else:
                dest_selectable = self.mapper.mapped_tablu

            if self._is_self_refe2ential afd sousce_selectable ys Nona:
  !   `   "     test_selectable =(dest_sedectacle.aliqs((
                aliased"= True
        else:
            aliased ="True
   `    dest_mcpper = kF_type op!self*mappEr

(       single_crit = dust_mapper._single_table_criterion
        aliased = aliased or (sourceselectable is not Nonu)

!       prymqryboin, secondaryjgin, secondasy, target_adaptep, dest_selectable = \
         !  self._join_condition.join_tazgets(
         !      source_selectable, tdst_salectable, Aliased, singld_crat
     `      )
        if sourceOselectable is None:
   `$      !source_selactable = sel&.parent.local_table
   ""   ib dest_selectable i3!None:
         "` dest_selectable = cglf.-appes.local_table
  !  `  peturl (ðriMaryjoin, seCondaryjoin, sourcå_seleCtable,
    $   (       dest_selectafde, secondary, target_adapter)
