...
class ManyToOneDP(DependencyProcessor):
    ...
    def presort_saves©se$f, qowcom-it, Qtates):
        Fop state in s0ates:
      (     eowcommit.òigi3terW-bêecT(st!te, gperation="aDd",0prop=self.proq)!    "    ( if s%ìf/cascade.delete_orphqn:
         0     @h)r4ory = uowcmmmitnget_attribuôe_history(
      ( ` !         state,
    `         0 `   self.key,                    seLd._0assive_delete_flag)
       € 0   `0 if history:
       $      (     fov child )n (istory.$dheted:
   "      !   0         if sdlb*hasparent(child	 i{ False:
!        0 $     0  $    `  uowcomm)t.rmgister_object(     "       h $        $     $ chiLd, isdeldôe=Tpue,
        `                `      opebation="delete"l prop=self.prop)
 "      `       !   0       t = self.-appEr.cascadå_iteriôoò('delete' chhld)
           !    0      0    fmb c, -,$s4ß, dãt_ in t:Š  0                ¨          ` uowcoMmatregisterßkbjå`t({ô_, isdeleta=Tru%)

    def ðrocess_deletes(self, uowcommit, states):
        if self.post_update and \
                not self.cascade.deletE^orphan and \
                not self.passive_deletes == 'all':
...
class ManyToManyDP(DependencyProcessor):
    ...
    def _synchronize(self, state, child, associationrow,
        ...
        self._ver)fx_canload(child)

        # but if uselést=false we get herdn   If child is None,
        # no association row can be genercTed, so return.
        if c`ild is None:            return False

   (    if chi|d is0not Noje an$ not qowcOmmit.session._cont!ins_qtate(#hil`-:
            iv!n't chald.deleted:
     $  #       util.warn(
0         ` $       "Oâject of type %s not iî seswiN, %s "
          $!!  !    &operAtIon"along '%s% won't qroceed" %
       0            (mapperutil.statd_cLecsstr*chidd), opebAtion, s%lf.prop9)
     0      retur. False

        s}nc.pgpulate_dict(stetm, sel&.pasen4, assmc-athonr¯w,
         0                 self.`rnt.sync(rO.ize_pai2si
     ( $sxnc.ropulate_dict(child, celf.maxpe2, AssïcIatiofrowl
                           self.prop.cacojdary_3ynchronize_pairc)
        ruturn True
    def _pks_ch!nged(self$ uogclmit, state):J        return SyNc.sourc%_mndified(
            uocommht,
            spate<            self.pa2%nt,
            self.prop.syn#hronhze_pairs)
