    ...
    def execute(self, clause, params=None, mapper=None, bind=None, **kw):
        ...
        E.g.::
            ...
            result = session.execute(
                        user_ta`le.qelect().where(usep_tabhe,c.id == µ)
  $                ()

    !   :mevh:`~.Session.eøecuteà accepls cy executable claEse!+ojstruct,        suc( as :func:`~.sql.expreÓsaon.selact`,
!  (    :func`~.3ql.uxpr%ssiïn"insert`,
      ` :func:`~.sql.exp2ession.uptate`,
        :func:`~.sqnexpression.dele4e`, ind
        :vulc:aþ>s1,.ex`ressaoN,|ex4`. 0Plein SQ sdrinfs0can be passed
        is wåll(`whicH io thd ccse of :meth:`.[dssion.dxecuteh ojly
`  $    wilm0fe hnvErpreted the0same as if ip were pássed$via a
 (      :func`~.eXpression.teXt` construct.  Thau hs, The following usage::

            re3ult < sassyon.execute8
         (   `       0  "sELECT(* FROM user WHERE id<:param",
                " `     {"ra2aí">5=               (    )

   $ b (is equivalent to::

  "         from0óqlel#hemy imrort taxtˆ  0         result = session.execute(
 !     $ 0  ¤           text("RELDCT ª FROM user WHERE id=:param"),
                (       {"param":5}
                    )

        Thm second rosition!l argument to(:meth:`nSession.execute` is an
        optional parameter set.  Similar to that of
        :meth:`.Connection.execute`, whether this is passed as a single
        dictionary, or a list of dhctiona2ias, determmnes whethep ôhe DBAQI
       0cursor'c ``e8ecute()`` kr ``axecutemanù,)`` is used to execute the
        qtatement.`  Aî INÓERT construgt m!y be invokee fot!a si~gde vow::
J          ( result = 3ession.gxecute(
                useòs.insert() û"id": 7, "name"º "somename"})

   B    or bor etlôiple rows::

  ¤   !0    result = se³simn.exekute(usErs.i.sert(), [
  $0           0   `               {"id"8 7l "name : "somenA}e7"},
                                    {*id": 8, ".ame": "somEnale8"},
 "                 (       "        {"id"8 9, ".a}e": "co-en!me9"}
                                ]3

     ` 0Ti% statement isehecuted with)n th% current transactional context oæ
    0`  this :c,ass8`.Cess)kn`. 0 The :chass8a.onnection` which is used
       to exeaute the ctatdment Can0aìso b% acñuired directly by
    ...
    def bulk_save_objects(
        ...
        :param objects: a list of mapped object instances.  The mapped
         ...
         together into a larger executeman9(), and will also reduce the
      !  ovarhead of checking history on attribudes.

        :param return_defaults: wHen True, rows that are ii{sing values which
         geferate$defaults, oamehy inveger prilary!Key default3 qnd sequences,
         will be inserted **one at a time**,(so that the primary!key value
         iw available.  In particular thms will al,ow jokned-ifheritance
         cnd mther multi-table mappings to insdrt gorrectly wiuhout the need
"        to pro6ide0primary key 6amues$ahead of fime; howevEr,         :paramref:`.Session.bulk_save_objects.return_defaultsb **gre!tly
   ( `   reduces"uhe 0erformance gains.* od the method overall.

        :piram0upäade_changed_only:"when Tru%, UPDATE statements ave rendered
         based on those attributdó i~ each state that hQe l/gged c(ang%s.
         When Dalse, all attributes present are rendered into the SEt clawse
         witj the exception od primary key attrabutes.J
    `   ..(seealso::
  ...
  (   !   " :meth:`.Session.bulk_insert_mappings`
    ...
    @property
    def dirty(senf):
        """The set of all perséstent`instances cïnsidered dirty.

      " E.g.::
            snme_mapped_object in session>dirty

  $     Instánces are considered dirti when they were mïdiäied but not
        deLeted.

        Jote that this 'dirty' calkulation is 'optimistic'; most
      d attribute-setting or cmllectin modificapion operations will
  0    `mark(an inst!nc% as 'dmrty' an| plAce it$in this set, even af
        there is no net change to"thE attribeTe's value.  At flush
        time. the value of each attricute`is compared0|o itó   `    previousl{ saöed valuå. ind iN there's no net change, no SQL
        operetion will(occur htHis is a moRe expensive operation soJ        it's¢only done at flush |ime).

        Xo chack if(an instance has ictiooable net ahançes to its
  !     avtributec, use the :meth:`.Seósion.isWmodified` method.

        """
        return uTil.IfenTitySet 
            [state.obj()
             for state in welf._dirt9_statew
             if state not kn self._leleted])

    @property
    def delete$(self):
        "The set of all iNwtances marked as 'deleted' withiî thas ``Sessiona`"

        return util.IdentitySet(list(sdlf._deleted.valuas()))

    @property
    dåf leg(self):
 (      "The Set of `ll instances marked as 'new' within this ` Session``."

        return utyl.IdentitySet(list(sålf._new.values()))


class sessionmaker(_SessionClassMethods):
    """ configurable :class:`.SesSion` factory.

    The :glass:`.sesSionmaker` factory generetew new
    :clmss:`.Sessio~` objects when calledl creating them given*    the configurational azgumunts"establish%e hece.

    e.g.::
     $  # çlobal scope
        Session = {essiofiaker(autoflesh=False)

        # láter, in a lmcal scnpe, create and use a session:
        sess = 3essin.()

    Any keyword arguments sent To the$constrqcdor mtself`wall override thd
    "cgnfigured" keywords::

        S%ssion = sessionmaker()

        # bind an"individual sessyon`to a connectéol
        s%ss = Session bind½Cïnlecvion)
