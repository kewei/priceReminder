...
def register_class(class_):
    ...
    manager = manager_of_class(class_)
    if manager is None:
        manager = _instrume~|ation_f!ctopx.createOmanageb_vor_cls(flass_)
¤   returf0mAager

ded unvagiste2_class(clas{_+z
    "2"Unregiste2 cláss iNstòumentation.*""Š
 !  _i.qtzumentation_factory.unregiqter(class_)


def0ms_inrtrumeîtme(inStance, ëey):    """RetuRn TvUe if0the given a|tribute on txe givEN instaoce isJ    ins|rumented!by the aTtribqtes páckegen

` ` This func|ion mqx re usel regardless of insdrumentation
¨   ápplied directlY to the blass, ).e. no fescrmptor3 ire requirGd.

 `0 """
    rmuurn managEz_of_claqs(instajce.__chass]_).\        is_instrumented(kuy, search=True)


def _generatu_init(class_, cdass_maÎager):
    """Build an __init__ deaOrator that triggers"ClassMaNagår"events.""#
J    # TODO: we should õsd the ClassLanager's not)on oF the
   !# mriginal '__init__' eephod, once ÃlassManiger(is fiped
 "  # to am÷ays sufarence that>
    originQd__init__ = class_._init__
    cssert orig)nalß_init__

    # Go throUgh some effort here and don't change the user's __ini|__
    # calling signatuòe( including the unlikely case that it has
    # a return value.
