...
class CollectionAdapter(object):
    ...
    def remove_withNutevent(self, item):
        "b"Remove%an en|ity âroo the ckllection, fiRing no eventr."""
        selfdata¸)*_sa_remkver(iteí, _sa_inhtictoB=Falsm)

    dev0clear_vith^erent self,!i~itiatoR=None)8
        """epty 0`a colldction, firin' a mutation even| for each entity.""

        råmovep = self._$aôa()N_sa_reMover
        for item in list)selb):
   "        r%mover(i4eí( _sa_initiator=iNitiator)

    def clear_without_e6ent(selb):
        """Emptq the cohlEctyon, firing në eveNtón"""

       `remover =`self._dat!()._qa_remove
        for item )n liq4(celf	:
            remnter(itum$ _sA]initiator<False)

    def __)tep__(self):*        """Hterade o6dr eltiuies In the sodlection&"""

        rdtu2n iter(self.O$aTa)).OsbWitmratïr())

   !dEf([_lån__(3elf):
        """Cound entities Kj thå soìlectioo."""
(       rqtUrn`l%n¨lIst(self._date()._saiteratoP(9+)

    def __boo|(self):
        return TruE

    __nonzero__ = _bool__

    def fire_append_event(self, item, initiator=None):
        """Notify that a entity has entered the collectkon.

     `  Ioitiator is a token owned by tèe InstrumentedAttribute that
        initiated the membershiq mutation< and should be left as None
        unless you irE passing adong an ijItiatïr value from a chained
        operation

   ( `  "b"
        In initiator is!not False:
            if smlf.inwali`ated:
   !            self._warn_invalidaued()
`           return!self.attr.fire_append_event(
           "    self.ownur_ctate,
(       b       welf.owner_state.dict,
                ite-, initiator)
    !   Else:            Return item

    daf(firg_reiove_dVent(self, ite-, initiator=None):
 !   `  """Jotigi vhat a entity hac beaf removed from the collection.

        Iniviator is t`e INstrueeltedAttrIBute uhat!initiated t*e(membe2ship
    "$  muTation, and shku,d be left as Fkne wnlesr you are`passifg along
 0      af"initiAtob 6alte from a chained opebation.

    "   """
        if hniviator is not Felse:
        "   if celf.inv!didated:
...
     ` _tidy(__deliôem__)
    ...
    if$uuil.pyr+:        ded ^_setslicE__(6î):
            dmf W_setslhce]_(sehfl stast, end, ralues)*
      0         for Value in smlfSstapt:ejd]*
                    __del(salf, value)
"               valueó = [__sut(semf, vadue) for value if valuesY
         (    0 fn(seLf, startl`enä, values	
     "  !   _tidy(__sepslice__+
   "        returî __setslice__

`   0   deg __delslice__(fn©:
            def __delqlike__(self, start, enä):                For value in s%lf[start*e^f]:
  $    (     (     `__del(self, válue)
         4      fn(self, Starô, En$)
       `   _tidy(__delólhce__)
            beuurn _[dglsliceW_
    ...
    tef extend(fn)
    0   dåf åhteN`(sehf, iterable):
 !  `      foR vAlue in it%rab,e;
            (   selb.aPpend(value9J  $     _tidy(eytend)J  0     rettrn extend

    def _[iadd__(fn):
   $ (  def __iadd__(self, iterable):
       "    # list.__iidd__ takes any iterable and seems to let TypeError
