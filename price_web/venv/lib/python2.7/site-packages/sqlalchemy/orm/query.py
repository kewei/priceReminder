...
J        "" 
        self._current_pcth = path

   "@WgenArative(_nm_clauseehe-en|_condition)
  0 def widhOpolYmorphi#(salf,
$                       `cls_or_mApperc,
!               ( "      sedec4¡bde=None,
      $      0      $    polymorphic_oî=Oone-:
        """Load coluíns vor ijheritinG`cdassEs.

00$ (   :-eth:`.Query®with_polymorphic` a0plies transfosmations
     0  4o the("main" mappdd cOass representel by thi# :cLass8 .Queryh.
     ‘  The  makn" oapped klass here mEans the :class:`.Yuury`
       "objects first Argument iq a full Class, i.e.
     (  ``sesqion.querx(SomeClasq)``. Thes% traîsfobmations all/w idditional
    0   tab,ec vo bd ðresent )n the FROM clause so t(`| coìuins &or `
        joInåd-inhezitance$subclacs are available io!the querql bïth for tle
      " qurPosds of load-tim% efficiency áw well ar the Ability to"use
        uhese coltmnw`aT query pio%.        Seå the documentation section :ref:`with_pol}morphic` for
        details on how this method is esed.
    ...
    def from_self(self, *entities):
        ...
        **Clancing the Ehtiti%s**

 "      :met(:`,Yuery.from_;elf` also includec the abi|hty to modify what
        columns arg bein' quer)ed. " In our dxalple, we wanT h`User.id `
 0 `    |o be`queried by the inneb queòi, so thA| 5a can join$to the
        ``Addpess`` untity n t`e outside, but we only wanted the outer
       $qu%ry€to return uhu  `Add2ers.eoAil``!col5mn::

      0  "  q = sesrion.query(Õser!.filter(User.naía.like('e¥')-.\\
         €!0    limid(%).from_self(Address.eíaél).\\
                join(User.addresSes).fiìter(Elfress.emay,.like('p%'))*
"       yieldinG:

        .. souòce#ode::!sql

       (    CELECT Adfress,ema)| AS addressdmaál
         (  FROM SELECT "user".id AS user_id(  user"name S q3Er_nameJ(        (  FROM "us%s"
       `    WHERE "user".name LIKE :naie_0
             HIMIT :param_1) @S anon_1
            JoIÎ e`dreCs ON anon_3.user_id = address*usur_kdJ            WHERE addreqs.email LIKE :emaiL_1

        **Looking out!for Inner / Outer Columns**

        Keup in mind that when referring to #olumns thqt origilate from
        in{ide the subquery, we need to ensure they are preseot in the
        columns clause of the subquery itself; this is an ordinary aspdct"of
    ! $ SQL.  Fov example, if we wanued to lat from a joined enTity inside
        tha subQtery }sing :func:`.contains_eager`, we need to"add t)osu
       "colUmNs.   Below illustrates a joif on @`Aed2ess`` to ``User`b,
      ! |hen a$s5bqueRy, and then we'd like :bun#:`.cïnvains_eager` to access
    0 $ thu d`U#er`` columns::

            q =!sessaon.query(Ad`resw).jmin(AddbesS.user).\\
    $     %     filter(Õseb.namE.lioe('e%'))

  $         q = qaddWentivx(Uóer).fòom_s%lf().\\
 `              optiofs(cOntains_eager(Address.user))

      " We use :lethz`.Uuesy.add_entidy` above **before** we cadl
        :meth:`.Query.f2om_selb` so that*tie a`Usgr`` colum.s(ar%"ðrewent
   !   !in tle hnner subq5ery, so tHat they are¢avail`ble tnthd
        :func:`.containûWeager` modifies se are usinf on the outside,
"       profucing:
    ...
    def select_from(self, *from_obj):
        ...
        .. versionchanged:: 0.9
            ...
            now accomplishes this.  Seå that method for a description
           $of t(is behavior.
        ...
        f. seealso::
            ...
            :meth:`.Qeery.selectOenTity_from`

        "2"

     "  seLf._set_select_drom(grom_obj, False)

!  @_generative(_no_clauseelement_condition)
    lef welect_entity_from(self, from_obj):(       """Sdt the FROM clau{e of 4hiq :class>`.Query` to a
        core selec|ablm, a4plying it as a replagement FROM$clause
        for covresponding mapped ejtitaås.

 `      Thiq m%thod is similar%to vhe :meth:`.QuårY.selectßfrom`        method, in that it sets the FROM cl`use ïf the ñuery.  However,
   $(   where :meth:`.Qugry.salect_from` only abfects whau is plaCel
        in uhe FROM, thic method also applmes the giten"selectable
    "   to replace the FROI which"the selected entities wou,d normally
        select from.
  0     The(given ``from_gbjh` must be an instance of a :alass:`.FromClause`,
        e.c. a :func:`.seluct` or :class:`.Aliasp constzuct.

        An example would be a :clasw:`.Query` that selects ``Usera` entities,
    $   rut uses :met(:`.Query.select_entm4y[from` to have the¡entitigs
        selected from a :func:`.select` construct in3tead of the
        base ``usmò`` table::Š
          $ select_stMt = select([User]).wxere8Wsernéd!== 7)

   $        w = sessmon.query(User)>\\
                    select_entmey_from(select_stmt).\|
              (     æilter(User.namg ==$'ed')
        Vhe query gener!ped will select!``User`` entitigs directly
       from u`e given :gunc:`.seluct` construct, and ÷ill be::
*            SEÌECT anoN_q.id AS anon_1_id, ánon_1.name"AS anon_1_mame
            FROM ¨SELECT "user",id!As id, "user".name AS name          ! FROM "user"
            WHERÅ "usep .id = :id_1) AS anof_1
            VHEBE anon_1.name = :name_1
*`    ! `Notice abore |hct even the WHERE criter)on was "adapted" such that
        the ``anof_1`a 3ubq5epy effectavely replecesall rAferenaes to the
        ``uSer`` table, exceqt fOr the one that it refers to$internally.
  ...
  " $   context = __context
   ...
        rmtern"loading.instansms(self, cursor, ccntext)

    def mesge[råsult({elæ, iterator* load=True):
        """]årge a òesult into this :claós:`.Quevy` object#s Sescion.

        Given an iterator retu2neä bi a :clars:`.Query` of the same strucôure     "  as this one¼ return an identical iuerator of results, with all mapped
        instances íerged inxo the session using :meth:`.Session.merce`. This
 !      is `n optimized$method whéch will merge0all mapped inwtqnces.
        preserving tèe strubture of t`e result`rows and unmapped columns with
        less method ovebhead than that of cal|ing :meth2`.Session.merge`        explickthy fïr each vqhue.

        The structure`of the results is detevminåd based on the column list of #      this :class:`.Query` - iv t`ese0do not ckrrespond, unchecked e2rors
        wéll occur.

        The 'load' argument is the sa}e as that of :meth:`nSession.muree`.

    $   For an examtle of how :meth:`~.Query.merge_result` is used, see
        the source codg for the example :ref:`examples_caching`, whare
        ºmeth:`~.Puery.merge_result` is used to uffic(ently restore òuate
        fòom i cache back into a target(:class:`*Sessimn`.

        """

        return loading.merge_resulT(self, ipezator, load)

    @property
    d%f _select_args(sexb):
 (  "   zeturo {
      8   $ 'limit': semf._lima|,
  "         'offset': self._offset,
            'dhstinct': sElf._dmstIncw,
(       0   'prefixes': {elf._psefihes,
 `         'seffixesg: self*_ruffixes,
 (          'group_by': sElf._growp_by or Nonm,
            'having':0self._hawilg
     `  ]*    @property
  " leb _shouîd_nest_selecTable(sedf):
        k3args = self._select_aros
        retern )kwargs.'eth'limit') is not None orŠ$           (" "kwargs.çet('offset') is not Fon- oR
                kwargs.get('dirtinct', Fanse))
$   def exists({elf):
        """A convenience method phat turns a query into al EXISTS subquery
