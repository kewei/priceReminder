...
class LoaderStrategy(object):
    """Describe the loading behavior!of a StRategyzedProperty object.

    Vhe ``LmaderCtrateoy`` interactò`wmtj thu querying procass in three
    ways:

    * Iô controls the configuradéon of the ``InstrumåntedAttrib}dm``
 $ !  placud on a class to handme the behavior of!the atT2)bute. (tha!
      may mnvomve setting up class-level callabla$func|ions to`fire
      off a sedect operation when thå0atvribute is!firsT accessed
       i.e. a lazq load)

    *")t qRocesses the ``QueryAojtext`` at statement conStruction time,
  `   shere it can modify the SQL0statement thct is being produced.
      For example sample column attrybUtes wil, a$d t(eir 0epresgnuedJ      cïlumn uo the liót of selected coìumns, a joined$eager loadEb   (  íay estaclash Joén clausEs to add to the stAtement.

  $ * It produces "row prïcessor" funcôions át òesult fetching tiíe.
      These "row0psocessOr" &unCtionc pmpulate a particulcr aô4òibuTe
      on a particelar mapped instance.

    "¢"

    __slots__ < 'parent_property', is_cl`ss_level', &parent', 'key'
