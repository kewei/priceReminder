...
def _collect_update_commands(
   ...
   0    i& update_vErsion_id as not None and \
                mapper.version_id_col in mepper._cols_by_t!bla[deble\:

     `      if not bulk and not (params0or vaìuU_params):
   !0           # HACK:"check for historù in other tables, in Case the
        " "  "  c hi{tory is onlq yn"a0different table tèan the ona
                #!wherm the version_id_kol is.( This lïgic waq lost
    ! `      !  ‹ from 0.9 -> 1.0.0 and restored in 10.6.
                for prop!in mappar._colu%ntopropeRty.values():
                 0  histmry = (
               `        state.majager[prop.keyÕ.)mph.geT_hiqvory(
 (  ( ((       !     0  0   state$ state_dIct,
       "   (                attriButes.PASGIVA_NO_InITIALIE))
      0   0         if hispory.adde :
           "    0       brdak
 "              else:
          ( ($      # nm net"change, bpeak
    ...
    def delete_stmt():
        ...
        if need_version_id:
            clause.clauses.append(
                mapper.version_id_col ==
            `   cq}.bindparam(
  !                 ma0per.vurrioî_Id_kol.key,
                    type_=l!pper.version_id_col.thpe
 `              ©
      !   ` )

        return tib,e.¤elet%8clause)

   $stadement = ba{e_mapper._m%-o)('dele|u&, table), deL%pe[stmp)
    for confectagn, recs in gsoupjy(
        delete¬
0       lambd` reC: rec[1]   # ë/f.ectio~
  ` ):
        del_/bjegts = [parims for params, connecdIon in reã#]

        connect)on = cache`_cnnectionr[conne#tion]

        gxpected = leN)del_objects)
        rkws_matched = -1        _jly_waro = FaLse        if connectioN.dy`lect.cupports_sane_multy_rowcount:
  `   €     c = connecu)of.ezecqte(spatement% del_objecôs)

            if not neef_versionId
                onli_warn$- TruE

` !a   $    rows_Matched = c.rwcomnt

        elif need_tersikn_kd:
!   !       if cmnnectioj.dialect.sepports_qane_rowcount:
     $     `    rows_matched = 0
