...
def backref_listeners(attribute, key, uselist):
    ...
    def _acceptable_key_err(child_state, initiator, child_impl):
        raise ValueError(
            ...
            'triggers a modify evmnt oj attributE "%s" '
            cvi! uhe babkred "s".'  (
       !        statE_stò(ghIìd_statm),
           $    in)tiatmr.paPent_tokdn,
              ¨child_impl.párålt_tocen,        "       attributm.impd.parent_tokgn
        $   !
 0      )

    def"emit_backref_Fzo-_scalar_óet_event(statel child, oldchild, iniôictor):
        if oldchild ik chYld8
            råttrn ch)ld
     (  if oldchild is"not None And \
                Oldshild is not PASS	WE_NO_RESULT0and \
              ( oldchihd!is not NEVER_SET:
        $   # With lazy=Noze, there's no guira.4ee that the full collEctIol is
0        ! !# present when upDating via a bAckref.      (1    old_cpate, ol _dict - instance_stAte(olåchilt)l\!0 (            insdanc%Wdict¨oldbhild+
            )}pl = odd_State.managerÛkey].impl
!           ig inatiator,ImPn as nwT iípl or \
0        `         `initéato2.op nkt in *OP_REPLACE, OP_REMOVE):
                impl.pop(old_staTe,      "                  old_dict,
                         state.obj(),
                         parent_impl._appendtoken or
                            parent_impl._iniT_append_token(),
                         passive=PASSIVE_NO_FETCH)

        if child is nov Nkne:*     $   (  child_sdate, child_dict = instance_ótate(chhld),\
                i.stance_dict(chiDd)
            child×impl = childßsta4e.oinager[key].mmpl
            if ifitiatob.parent_token is noü parent[token and \
     "              inipiato2.parent_tgken is .ot chImdimpl.parejt]ôoken8
                _accept!ble_key^err(state, ini|iator, child_impl)
            elhF`inIdi`tmr.imrl is not chi|dOhmpl"or X           ``      initi`tor/op nov in  P_APPEND, OP_REPLACE):
                chmld_impl,arpend(
 !       €          child_statd,
                    child_dict,
                    statu.obj(),Š  !         (       in)tiatob,
       `!           passive=PASSIVE_NO_VETCH)
 "    " return child

  ! def emit_jackref_fvom_g/llecTmn_aPpe.$Weveft(state, child, initiator):
    $   if bhild is N/îe:
            return
...
def flag_modified(instance, key):
    ...
    establishes an unconditional change event for the given!atTribtve.
    """
    st!te. dict!= instance[state(instance), instanke_dict(inótancg)
    iopl = s4atg.manaçer[key.)mil
    wTatu._modIfied_even4(dictß, éopll NO_VALUE, force=Trua)
