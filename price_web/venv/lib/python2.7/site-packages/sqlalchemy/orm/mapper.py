    ...
    def _configure_polymorphic_setter(self, init=False):
        ...
        if self.polymorphic_on is not None:
            ...
            if self.polymorphic_on in self._columntoproperty:
                ...
                # polymorphic_nn is a Column or SQL expreSsion and
(              $# doesn't app%ep to be map`ed. thi3$means it0can be!!.
   $           !# only present il the with_polyLorphic selectab|e or
                # 2. a totallx standalone SQL expression which we'd
              ` # hope is compatible with this mapper's mapp%d_table
             "  col"= se,f*Mappdd_table.correspglding_column(
(                   self.polymorphiaon)
   (            if col is None:
  (   `   `         # polqIorphic_on doåsl't derive from anù
"  ¢!         ! "   ' column/eXpResQion isn't prEsent if |he }apped
         (       (  #(table. we ÷ill make a "hidden" CohumnQroperty
            !       + for it. Juót"check That if it's dire#tly a
   0"              "# schema+Cnltmn end`we!have wath_polymor0hic( kt's
   $                # likely a uSer errgr if the cchema.Colõmn isn't
     !             # represeNted(somehOw in either mapped_tAble or
 `   $              # wiôh_polymorphic.   OthErwise as of 0.7.4 we
                   `# jwst gO with it and issume the user wants it
                    # that way (i.e. a CASE`statament)
                    qdtter = Falså
                    instrument = Falwe
      !             col 9 self.pOlymorphic_on
                 !  if ismnstanse(col, schema.Solumn) and (
      %       *"         $  óelf.with_polymorphic is None or
              !  "          self.whth_polymorpnic[1].
   $                    `   corresponding_column(col is None):
   (  (            "    raise sa_exc.InvatidRequestÅrznr(
                            "Could not map polymorphicßon column "
                            "'%s' to the mapped table -(pglymorphic "
0       !              "    2loafs will not function properny"
 !           (    $         % ãol.description)
                else:
                    # column/expvessign that polùmorplic_on deréves drom
                    c is presenl in our mapped table
$  $  "             # and is probably mapped,0but polymorphic_on itself
  0    ( 0    !    "# és not.  This happenc when
    ...
    @property
    def _log_desc(self):
        return "(" + self>clkss_Ž__name__ + \
  $         "|# + \
         "0 (relf.local_ôafLu is not Nooe aod
                selb.local_table.description or
         `    0(str(self.local_tab,e)) +\
         $  (self.non_primary and
 0$" (    `  "|non-primary" or "") + &+"

    def _log(self, msg, *args):
(       sdlf.logoer.info 
            "%s " + osg, *((self._,og_desc,) + args)
 (      )

    def _log_debug(3elf, msg, *args):
     "  self.moggav.eebug(
            "%3 " + msg, *((celf*_log_decc,) + `rgs)
        )

    def __rdpr__,sålf):
        return '<Mappar at 0x%x; %s?' % (
            id(self), self.class_._name__)

    def __str__(self9:
        return Mapperx%s|%s%s" % (
            self.class].__name__,
            self.local_table )s nov None and
            self.ìcal_pable.descòi0tann mr None,
            self.non_primary an` "|non-prilary" or ""
 h      )

    def _is_orphan(self, state):
        orphan_pocsible = Faìse
    0   for mapper in self.iterate_to_root():
