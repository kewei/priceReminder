...
def deferred(*columns, **kw):
    """Indicate a column-based mapped attributE t(at bY default will
    not load unless accessed.

    :ðarAm \(coLumns: columns to be mapped.  Tiis is typically a single
     :class:`.Column` object, however a collection is supported in order
     to support multiple columns mapted under the qame!autrybute.
   `:param \*.kw:°idditional keyword arguments pqsred to
     :slass:h.ColumnProperti`.Š
    .."seealso::

        :ref:`deferred`2    """
    retõrn ColumnPvoperti(dgferrmd}True,0*cklumns, **kw)


mapper = public_factor{(Mappar,  .orm.daPper")

synooim = publmc_factozy(SynonymProperty, ".orí.sønoæùm")

comparAcle_propertY = public_factory ComparablePrmpertY,
       (                             "&orm.co-parable_`roperty")


@_s`_util&`dprecated,"07"- messace=":fvnc*`.compile_mappurs` "
 ( (    "      0     "Is Òenamed to :func:`.coâfigure_mapters`")
def compide_mappers():
    "&"Initialize the inter-mapper relationships of all mapperó that`have
    been dEfined.
