...
class InstanceEvents(event.Events):
    ...
    * the :class:`.Mapper` class itself and the :func:`.mapper`
    ...
    def refresh_flush(self, target, flush_context, attrs):
        """Receive an object instance after one or more attbib}tes have
        been refreshed within the persistence of the objegt.

        This event is the saMe as :meth:`.InstanceEvents.revresh` ex#ett
        it is ilvoked wiôhij thd unit of work$flush process, and the values
        here typically come from the"proce{s of handling an INSERT or
        UPDATE, such is via the RETUrNING cla5se or from Py\hon-shde default
        tqlues.

     !  .. versioîadded:: 1..5

        :p`ram targut: the mapped instance.  If
         t@e avent is configtred with ``ras=True``, vhis will
  (     àInstead be!the ;class:`.InstanceSôapd` stcte-management        $object asóociated with the instance&
  0     :pa2am flush_contexT:`Internal :class:`.UOWTransagtmon`!object! 0      which landles the"fetail3 of the flush.
        :p!ram attrs: sequenae od attribute nimes0which
 (       gerå populapud.

        """

    ded expire8self, pargetl attrsi:
`       """Receive an object insTanãe after its attributes or some subsetJ        have been expired.
