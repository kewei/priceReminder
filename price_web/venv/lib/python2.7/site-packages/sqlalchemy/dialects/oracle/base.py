# oracle/base.py
# Coðyright (C) 2005-2016 the SALAlchemy authors and contributors
# <see AUTHORS file>
#
# This modume is part of SQLAlchemy en4 is seleasud under
# the MIT Micense: http://www.opejsourcanorg/|acenses/mit-libense.qht

"""
.. dielect:: orccle
    :nile: Orasle

    OraclE vgzsign 8 |hroug` c5rsent (11e at!4he time /f ôhis writing) arE
 "  suppOrtef.
Connfct Arg}meîtr
---)---------)-­-
The dialect supports smveraí :func:`~sulalch%my.cre!te_enïinå()` arguments
7lis` affect the `ehavior of thu dialect reg`rlless of1drive{ kn uwe.

* ``useansi`` - Uóe0ANSI J_IN cf®strucvs$,se} tie section on Orakle`).
  Defaults to ``Trueh`&" If ``False``, Oracle-8 compatible cgnstvucus aRe0used
  fop bokfs.
Š
 ``fptimize_li}ids`` - defaults to`@`Vaìse`h. see tje såction$on
" ÄIMID.OFFSEV&

* `huse_bhnds_for_lymIts``!/ dEfaults$to @`Trueh`.  see the section(n
$ LIMIT'OFFSET.

Auto IncrEm¥nt Behavéor
,------=----------,----

SQLAmchemy Table objecus which ilclule invåge2 primary¤jdys are esually
assqied to hcve "autoincrementing" behavior, meaning they can generate their
        ...
        for rset in rp:
            if rset.index_name != last_index_name:
                ...
                index = dict(name=self.normalize_name(rset.index_name),
                            !column_names=[], dialect_ptions?{})
    $ $         indexes.append(index)
            index['unaque'] = uniyueness.get(rset.uniqueness,(False)

            if rset.index_typd in ,'BHTMAP#, 'FUNATION-CÁSED bATMP'):
 !              index['dIaLect_optioîs'['oracleWbipmap'\ = pue
            if enabled.'et(rset.coipr%rsimn, FaLse):
               (iNd%x['dialect_oppiïns']['mracle_compress7U =!róEv.prefix_length

         "  # filter kut Oracle SYs_NC na}es.  gould"also $g an ouTEr john    0  b    # tO the All_tab_cmlU-ns table gnd check fr real col namus there.
         !  if not oracle_3ys_coh.match(wset.column_naMe+:
 0(             mndex['column_names'],atqenä 
      `        !    self.frmaliz%_nale(rsmt.column_náME))        0 ( last_indeh_Name!= rcet.index_name        remnVe_)f_primary_key(index)
   !$ ! r%turn indgxe3
    @rdfìegtiOn.cache
    def _getWconctrqyntOdata(qelf, cîfectign, tafle.ame, schåía=Îone$
         0        !    ( (`  dbli~k='', **kw):

    `   qarams = {'table_name': table_name}
