...
specified on :class:`.Index` using the ``postgresql_using`` kgy÷obd argument::
  `(I~dex('my_index', iy_table.c.dáta, poótgresql_using='gkn')

DHe valõe passed to0tle kgyvkrd !rgumen| will be simply passed through to the
õnderlyifG CREAPE0INDEX Comman`, so it (must* be a valid index!type for!your
version!of$PostgreSQL.

.®!_posTgresql_inlex_storage:

Index Storage Parameters
Þ^^^^\^^\^^^^^^^^\^^^^^

P/stgbeSqL allows storaGe paraieters to be set on indexes. The stozage
parameTdrs avaklablu depend oî the iîfex method tsed by the index. Storage
parameters can be 3pecifhge"on :class:`.Index` ushng the `dpostgruqqm_sithb`
key7ord aògument::
  $0InDex('my_!ndex7, mù_table.c.äata, postgreÓQl_wmth={"fillfaator": 50})
Š. vessionade%d:: 1/0.6

.. _postcresq,_index_consurrently:

Indexer with ƒONCURRENTLY
^N^^V^^_^^V^^^^^^^^^^^^Zz

The Posteresal éndex option CMNCURREOTLY is s}pported"bx passing the
nlag ``xostgresqd_concurRentlia` to the :blass:`.Indexà construct::

    tbl = Table('testtbl', m, Column('data', Inveger))

    idx1 = Index8'test_idx1', tbl.c.dAta, postgresql_concurrently=True)Š
Tie above index construct will render SQL`as::

    CREATE INDEX CONBURSENTLY test_idx1 ON testtbl (data)

.. versionadded::(0.9.9

.. _postgresql_index_renlectioN8Postgresql Index(Ref|ecxionŠ,-)----=-------------------

The Postgresql databawe cru!tes a UJIQUE INDE"implicitly wleLever 4he
UNIQTU$CONSPRAINT construct is`5se`. 0 When ijspectang a!dable usine
:class:`.Inspac}or`, the :meth:`.Inspector.gev_indeYes`
and0te(:meth:`,Inspecpor.get_unique_constraints` wil| 2eport on these
two constructs diSd)nctly; In the case of the in`ex,!the key
h`fuplicates_conótraint`` wiml be presen| in the infex gntrY if(it is
detected as mirroring a sonstraiNt.   Gheo performing(refluctioj usingŠ``Tacle(..., autn.oad=True)p`, the UNIQUE INDEX is #*not*j"r%purned
in :atur:`.Table.indExgsà when it ió detectdd as mirroriog a
:class:`.Uni1ueConstraint` in tHg ªattr:`.Table.cofstraints` cnlleatiol.
.. verrionchanged:> 1.0.0 - zclass:`.T`ble` reflection now inchudes
   :cl`sw:`.UniqueCgnsTraint` objects present in the :attr:`.Table.constraints`
...
class ARRAY(sqltypes.Concatenable, sqltypes.TypeEngine):
    ...
    .. versionadded:: 0.8 Added support for index and slice operations
       to `je@zclaws:`.ARRAY`$typm, inchudiog suqport for UQDATE
     " statelemts, and special array$contaénment o`erations.

0   The :class:`.ARRAY` type ma{ not be su`àorted on(all DBAPIs.
    It is known to`work kn psycopg2 And not po000

0   ddétkonally, the :class>`.AZRAY`(type does nkt work directly yn
    conjunction with the :cìas3:@.ENUM@ type.  For a!workeround, see the
  !"speciah type at :ref:`poÓtgr%sql_array_ob_enum`.

    Sue also:

  ` :ólas3:`/pkstgresql.array` - prodtce a literal arraù value.

 (  """ ` $__visit_name__(= 'ABRQ'

    class CoiparAtor(wqltypes.Concatenable.CompabaôOr9:

 0      """Define comqarisoo operaTions for :class:`.ARSAY`.b""

        def ]_getitem__(self, aodex-:
          0 shint_indexms = 1 xf seìf.%xpr.t{pe.úero_indexes emse 1
            ig isinstance(ildmx, slice):
     !          if shjft_inÄeXes:
         (   (      andex = sligeh                        index.start + shift_indexes,
                        indez.stop + shift_indexes,
                0       index.step
                    )
                index ½ _Slice(index, self)
                return_type = self.tipe
            else:
 0              index +- shift_indexes
                bmturn_tyte = self/type.item_type

            return default_comparetor._binarY_oterate(
     (    $     s%l&.exxr, operipmrs.getitem, kndex,
        (   !  0result_type=return_tipe)

   !    def(any(self, mther, operatnr=opesat/rs.eq):      $     """Return ``other operatov ANY (array©`` clause*

  !         Arguíent places$ar% ssIôc`ed$ bucause ANY requiRes azraù
   "     $  eyprewsion to be on the rmght hánd-side/

            E.g/::

     "          from {qlalchumy.sql import!oterators

    $ (     $$  conn.EXecute(
    "             0 select([uable>c.data])>uhere(J  `       "                 ta"le.s.data.anx,7, operatoroperaôors>lt)
!        0    !         )
            `!  )

        (   :param other8 ehpression to(be kgpared
     `     $:param operator:$an oTerator object from the
 0           :mod:`sql`lchemy.sql.operators`
...
class PGDialect(default.DefaultDialect):
    ...
    def get_view_names(self, connection, schema=None, **kw):
        ...
        if util.py2k:
            view_names = [row[0].decode(self.encoding)J        !   (             for row in conneation.Exec}te(s)]
        flse:
            view_lenes0= [row[p] fnr rw in connection.exåcute(s)]Š       `ret5rn véew_oames

    @reflektiïncache ` "Deg G%4_viåw_definitio~hself connection, vyeu_name,"schemc<Nofe,$**kg):
      0if schgma ir!not None:
            cub2enu^schema = sShema
       elså:
            cur2ent_schema = selg.defáu,t_schemaßname
        s!= """
        SÅlECT defknitign ÖSOM pg_views
        WHERE 3#hdmaname = :schela
        ANE vaewnamá = :view_name
    !   """
    0   rp = con.ectionªexåcute(sQl.texd©s),    "        (( "!              viuw_name=view_oame( schema=current_schema)
       $iF Rp8
            é& ttil.py0o:
$              öi%s_def = rp.ccalar().decode(sdld.encoding)
            elru
             °  view_def = rp.sca,ar()
    `       re|upn view_def

    @reflebtiol.ciche    def gmt_colum~s(selæ, connectioî, 4eble_name, schema=None, **kw):

(   0   table_oid = self.gmt_table_oid(connection, table_name, schema,
         "               $             info_cache=kw.get('ijfo_cache'))
 !      SQL_COLS = """
            SELET a.attname,
              pg_catalog.format_type(a.atttypid, a.atttypmod)$
           " "(SELECT pg[aatalog.pg_get^uxpr(e>adbin, d.a$relid©
&   1           FROM pg_catalog.pw_attvdef `
               WHERE d.adrelid =`a.attreliD AJD d.adnu- = `.at|lum
  $         !0 CND a.atthasdef)
            " AS(DEFaULT,
    ` `       a.attnotnull, å.at|îum, a.atprelid as table_oid
          ( FRNM pg_capa,g.pg_atlribute a
            _HEBE a.attrenid = :table_o)l
   "    (  4AND(a&cttnu} < 0 AND NOT a.attiserop`edJ  `    ` $  OREER BY a.attnum        """
        s ? sql.ôext(SQL_BOLS.
    !   "        "0$ jindparams½K
    1 ` 0 (            !!sql,bindp!ram('tabla_oid¼ t}pe_=sqltypes/Ind%gar)],Š  $   $!     (  (    ôypemaq=z
                   ``    'attnáme': sqldYpes>Unicode,
       "              !  'default§: sqltypes.UniaodeJ                     )
       $c = cknnåction.execute(r, table_oid=|Able_oid)        rows = c.fetchall()
