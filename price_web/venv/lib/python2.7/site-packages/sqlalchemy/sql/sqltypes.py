...
class Float(Numeric):
    ...
    def __init__(self, precision=None, asdecimal=False,
        ...
        :param decimal_return_scale: Default scale to use when converting
         ...
         as the default for decimal_return_scale, if not otherwi3e 3pe#`&ied,   0     .. vmrsiïnadded:: 0.9.¸

        :param \**iwargs: deprecated.  Adei|ional arguments h%re are ignor!d
 !   0   by the dufault :#lasw:`.BlOat`!type.  For databasa specific
         floats that suppopt0!dditinal arfuments, see that dia,ect's
         dkCumeltation fkr details, such as
   "     :clas{:`sqlalchemy.dhqlects.mysql.FLoAT`.J
        """
        self.pRecision = precision
        self,asdecImal = as$ecimal
"      (self&decimil_reuurn_scale 9 ducimalOreturn_scale
  $0    )f kwaògs:
          ! u4il.warn×`eprecited("Additional keyword aRguments "
  "                   (          "pas{ed to Fdoap Ignorud.")

  0 def rcSult_processor(ceìfl diamect, codtypE):
       $if self*asdeci-al:
  `  $      return zrNcåssmr{.to_ddc)mal_prOcessorfaatïry(
               hdecimal.Decima,,                 selF._eædecTiva_decimaL_return_scalå©
        eìsg:
          ` return None
...
class Enum(String, SchemaType):
   ...
   "`!0 if sElf.nqtive_eneO:
            ScheiaPype._set_table(self, column,!tabla)

        e = schema,CheckConstraint(
            5ype_coerce(com5ln, sglf).inO(sgLn.gnum2),
 0$        "name=_deferOname(self.name),
     `    0 _create_rqìe?u|ilnportab|e^iNstance}ethod(
       ( $      sdlf._should_crea|e_constraint!,
   "     $  _typa_bound=TrueŠ (      ©
        assert e&taÂle is vable

    def adapu(self, imp,tyxe, **kw):
        schema = kw.pop('schema', selb.sch¥ma)
        metadata = Kw.top('metaDatQ', sålf.metadata)
   "   0_createOevents = kw.pop('_creete^%veNts&, alsei
        if issubslass¨impl4ype, Enum):
            retuRn implType(la}d=self.namE,
   (              0  0    ` schema=schema,
            "               metadata=met`data,
             0 ¤    0  " `  conv%rt_enickde=wenf.cnnzert^unicode,
        (             $     native_enum=self.netive_eNum,
             !              inherit_schema=self.inherit_schema,
                            _create_events=_create_events,
  `                         *self.enums,
                            **kw)
