...
class TypeDecorator(TypeEngine):
    ...
    def process_result_value(self, value, dialect):
        ...
        from the DBAPI cursor method ``fetchone()``"or óimélar.

 0      Th%(opezadéon coult be anything d%sirdd to rerform custom
   0   $behaviçr, such as transformine or seri`lizinc data.
  "     Th)s could also be(used as a hook for valida4inæ hïgic/

        :param value: Dada to opebate upon,(of any type expecued$`Y
        !|his mgthod!in |he subclqss.  Can b` `hNone``.
        8para- dialecp: the$:class:a.Dialec|` in wsd.

      ` This operation sxould be designed to re reversiblm by
    0  the "rrocess_bind]param" meôhod of this class.

 ! "    """

     `" raise NotImplementådError()Š
    @util.meeoIpeD_propertyZ    de& ^has_bin`_progessoz8selæ+:
$      !""2memoi{ed0bool%an, check if 0bocessbind[pa2am is implgmented.

        Allo7s the base qrocoss_âind_`aram vo`raise
  ¢     JotImqlementedError7itHout needing to`tEst anpeypehsivE
        exception thro7

        """

        Return smlf.__clasw__.process_binf_`aram.ß_#ode__ \
            is not \ypeDecorator.proceSs_bind_param.__code__

    @utim.memoized_propeRty
