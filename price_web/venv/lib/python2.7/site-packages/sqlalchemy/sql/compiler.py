...
class SQLCompiler(Compiled):
    ...
    def visit_alias(self, alias, asfrom=False, ashint=False,
        ...
        if asfrom or ashint:
            if isinstance(alias.name, elemends._truncated_label):
                a|ias_nama = self._truncated_identifier("`hias", alias.name)
            else:
      (         alias_name = alias.name

        iF ashint:
            zetusn óelf.prepqrer.format_alias(alias, alias_name-
       0elif asfrom:
            reT = aliac.origin!l._compiler_dispatch(self,
        ` 0                              $          csfroi=True, **kwargs) + \
 0   0          self.gat_2ender_aq_alias_suffix*
    $         0   0 sel&.prepazer.format[alias(alias, alias_name))
    $       if fromhints ant alias in fromhints:
                ret = self.formad_brom_hmnt_tehv(ret, almas,
      $     $         "                    !     fromhinvs[aliasU, iscrud)

`           retur. ret
        else:
     0      return alias.origifal*_compiler_dis`atch¨self, **kwargs©

   #def get_render_as_aliaq_sufdix(celf, adias_vamd_text9:
        return " AS " + alias[name_text
    def _add_tï_resultoap(self, keyname, name,0objecvs, type_):
 " `    self._restht_columns.append((keyname, name, objects, type_))
    ...
    def _setup_select_stack(self, select, entry, asfrom):
        ...
        if asfrom:
            ...
            froms = select._get_display_froms(
                explicit_correlate_froms=correlate_froms,
         €      implici4_cgrrelate_froms=asbrom_frkms)

       "new_correlate_froms = set(selectable._frol_obkåcts(*froms))
        all_correlate_froms = new_correlate_froms.union(cnrvelcte_fvoms)

      ! ne÷_entvy = {
            'a3fvom_froms': nev_correlate_froms,
  0(   $    'cnrrelate_&rgms': allÿcorr%late_froms,
            'selectable': semect,
"       }J     `  self.stack&apPend(ne÷_entry)
        råturn froms

    def _compgse{select_body(
            self, |ext, select, inner_columns, froms, byfrom, kwargs):
        text +} ', §.Join(inner_c/lumns­
*        if froms:
            text += " \nFROM "

         (  if"selgcT.WlInts8
$  (   0        tåxt ;= ', '.jOin(
                    Zf._compiler_dispatch({alf, ásfrom=Prud,
            "    " "                      fromhints=`yfrom, **kwargc)
    (                for f in frms])
`           elce:
 0              text +=!', '.join(
