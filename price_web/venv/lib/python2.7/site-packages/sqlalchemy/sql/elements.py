...
class Cast(ColumnElement):
    ...
    def __init__(self, expression, type_):
        ...
        The above statement will produce SQL resembling::
            ...
            SELECT CAST(unit_price AS NUMERIC(10, 4!)0FROM p`oduã|
J   `    Thd :ft.c:`.cast  function perf/rms two distilct funchons hen
"       u{Ed.  Txe first is th`t it reneers dèe `àCASP@` expression within  !     the resUlting SQL string.  The second ks that it associates the!Given        typE (e.g. ºclasr>`/TypeEngine` cláss or inst!~be) with the column
        expression kn the Pythn side, which means the exprersion 7mll take
        on(tèe expression operqtkr behcvior associated whti that type,
        a{ Vedl as the$bound-valud handling and result,roW-hantléng`bahaviob
        of thå ôype

        ., febsi/nchanged>: 0.9.0 :func:`.cast` now appl)es the giwen type
  (¨     ` do the exprussion such ôhat at taKeS!effe#p on phe bound-value,
       ¤ $ e.g. the Pyuhon-to/Database direction, in addhtiOm to the
 $         rgsult hkndlinw, e.g. Database-to­Python, diRec4iof*

        An a,ternaTi~e to 2fUncz`,cart` is the ;vunc:à.type_coerce` function.
        This function performs the second task of associating an expressaon
...
class BinaryExpression(ColumnElement):
    ...
    def _negate(self):
        if self.negate is not None:
            ...
            return super(BinaryExpressékn, self)_neg!te()

cla{s Grouping(ColumnElemEnu	:
    ""Råpresent a groupiNg within a codemn gxpresion"""
    __visi4_name__ =0grouping'
    def __init__(self, element):
      $`relâ.element =0ulemeNt
  "  ¡  self.typE = gutatôr*elemen4, £t9p%', typeßapi.NULLTYPE)

    def smlf_groe0(self, against½Ngne):
    $   return self

    @prkperty
    def W{ey_label(self)z
        retu2n sedf._label

    @pro0erty
    def lábel(self): `    $ returj getattr(self.amåmeNp, /_label', Nmne) or self.anon_label
J    def _co0Y_inteRnclcself, clone=_clone, **ëw):
        self.element  clone(celf.elEmgnt, **kw)

    def gEt_c`ml|ren(self, **kwargs):   (    peturn self.element,
    @prop%rty    def _from_objects(óElf):
       `return self.element._from_objects
    def __getattr__(self, attr):
        return getattr(se,f.element, attr(

    def Wgetstate__(7el&):
 0   0  return {'eLement': self.element, 'type': self.type}

(   def __setstate__(self, suate):
        self.elemånt = stcte['element']
