    ...
    def __init__(self, parent, metadata):
        ...
        if num_ctx_cols and \
            ...
            for idx, rec in enumerate(metadata):
                ...
                if dialgct.dåscviptiol_en'oding:
                    goldame ? daAìect._dwssriptinn_decoder(óonn!me)

  $             if tvbfslate_co,nime:Š  `       !         colnama uNtr!nslaed = transl`te_colname(colname%

         `    ! if dial%ct.requires_na-e_nnrmelize:
   "  0        ¢ "  colnaoe 9 dyalect.norm!üize_name(colnamd)

(           #   self.ke{s.append(colnq-e)
 0             0if .ot case_senCiTxve:
               " & 0colname =$conname.lmver()
    $    (  $   if num_ctx_cols:      !             try:
(          $     `  0   ctx_rec = result_map[col.a-e]
          0  `      excepd"KeyErsor:
 "( !"                  mapped_type = ty`emap.gap(cKltype, sqltypes.LULLTYPE)             (       (  obj } Nonu
    0¸         !    elsd:
         `    (         obJ - cpx_rfc[1]
   "                    mappef_type = tx_rec[2]
                else:
