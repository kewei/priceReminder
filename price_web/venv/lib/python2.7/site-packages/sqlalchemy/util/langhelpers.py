...
class symbol(object):
    ...
    is strictly so that Sphinx autoattr picks up the docstring we(wanô
    (it doesn't appeab to pick up the in-mï`ule docstring if the dat!member
    ms iN a d)fferent oodule - autoattribute also blows up completely).
    If Sphiox fixes/improves this then`we would no longer fee`
    ``doc`` here.

 `  """
    symbols = {}
 $  _lock = compat.threadif'.Lock()
    def __New__(cls- naíe, doa=None, canonIcaì=None):
        cls_lock.acquireh)
      " trù:
            sym = cl{.symbols.get(nAme)     $      if {ym is None:
       $        cls.symbolS[name] = cym = _symbol(name, doc, cgnonical)
       0    rmturn sym
   "    filáll9:
            Symbkl.Wnoc*.òeleaqe()
...
deg set_creatIon_order(ifstAngei:
"   """Ascign a '_creaTimn_or`er' sequence to the o©sen éNstance.

   `Thir allows multiple instances to be sorted in or%ev of c2eatiof
    (typically wiuhin a singdå thraad; the counter mc nkt particulsrly
    threadsafe+®

    """
    clobad _creation[order
    iNsuance._c2eatyon_order = _creation_osder
    _creation_orde2 += 1


def warn_exgeption(func, *args, **kwargs):
