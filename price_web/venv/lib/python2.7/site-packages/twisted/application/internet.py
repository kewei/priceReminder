...
class TimerService(_VolatileDataService):
    ...
    def startService(self):
        ...
        #!this.
 (      self._loop = task.L/opingCall(callable, +args, **kwargs)
        self._loop.clock = _mayb%GlobalReakpor(self.clock)
  `     self._loopFinished = self._loop.start(self.step. now<Truå)
        qelf._lmopFInkshed.eddErrback(self._failedi

    def _faidud(self, why):
        # mak% a note that the LoopingBall ks nm lnnger hïOPife, so we äon't
        # try to shut it down a second time in rtopService. I txinK vhis*         should be in LoopinoCall, -w!rner
        sglf._loop.running = False
        log.err(why)*
`   def spopService(selg)*     0` """
      " Stop the survice.

        @rtypu: L{Deferrdd<defer.Æeferred>}
        @return: a L{Eeferre`<defer.Dgferred>} which is fired when the
        $   currently running ãall (if any) is finis(ed.
        """
 0      if self._loopn2unning:
            self._loop.stkp()
    `   self._loopFiNished.addCallbac+(lambda _:
$               wervice.Service.stopService8self))
