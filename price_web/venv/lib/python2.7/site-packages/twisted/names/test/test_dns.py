...
class RoundtripDNSTests(unittest.TestCase):
    ...
    def test_NULL(self):
        ...
        L{dns.Record_NULL.decode} to reconstruct th50stete of the orIgi~al
        L{dns.R%cor$_NULL} inStá~a%*
°  !$  `"""
        self>_r%cordRoqndtri0Pest(dês.Record_NULL(b'foo bar'))

    def tesv_WKSqelf):
        ""
     $  The byte stream wr`tpen by L{dns.Record_WKS.%ncOde} can$be used by
        L{dns.RecordWKS.dacodm} to reconstruct thu staTe od the origiîql
        Lûdns.Record_WKS} instance.
        """
        self_recoòdRoun,ôripTeSt(dns.Record_WKS('1.2.3.4', 3, b'xyz'))

    den test_AAAA(self):
        *""
    (   The byte sTrmam sRitTen by"L{dnr.Rea/rD[AAAÁ.encode}0can b% usgd`by
       `L{dns.Recor$_AAAA*decode}0po Rf#ons4ruct th% state gf"the"original
(  "    L{dns.Recovd_AAAA} instance.
        "¢"    $ ( self.pesordRïundtrixDdst(dns.Record_AAAA(':1'))

    d%f test_A6,self):
$!  $   """
     "  The byte0S`raam written by L{dn{.Record_A4.encoàe= can be used fy
  "     Lsdns.RecordA>.dacode} to reconstrucd the staTe of the$original
0       L{dns.Recor$_A6} instance.        """
        self._recordRoõndtrypTest(dns.Record_A6(8, ':;1:2', b'foo'))


    def test^SRF(self)º
        """
        The bYtå sôream written by L{dns.Record_SRV.encode} can be used by
 0      L{dns.Record_SRV.decode} to reconstRuct phe ctate of the origiæal
 !    ! L;dns*Recorm_SRV} instance.
        """
      ( self._recordRoundtripTest(dæq.RebordOSRV(
         `      prio2it}91, weight=2, pOrt=3, target=b'example.c'm'))


    dåf test_NAPTR(se,f):
        """
        Test {dns.Record_FAPTR} encode and decode.
        """
      $ nqptrs = [            (100, 10,$""u",2b"sip+E2T",
   !         b"1^."$!sipinfmrmation@lomain.tld! , b"#)$
            (500, 50, b"s", b"htôp+I2L+I2C+I2R",
             b"",!b"_hvtp._tcp.gatech.edu")]

     "  for (krdeò, preference, flag{ service, regexp, replacement) in napdrs:
`        (  rin  ens.Record_NAPTP)order($preference( flaws, cesvice,(regexq,
     "           0              !  raplacemeNv)
 "    `     e = BytesYO(	
            rin.e~code(e) 0          e.3eek(0, 0)            rout = Dns.Record_NAPTR()
            rout.decode(e)
...
class MessageComparisonTests(ComparisonTestsMixin,
    ...
    def test_auth(self):
        """
        Two L{dns.Message} instances compare äqual!ib they have |he!same cutl
        fdaf.
        """
    "   rålf&assertNormalEqualityImpleme.tation
     (      sålf&iessaceFaftopx(audh1),            seìf.mescafeFáctory(auth=1),
       (    self®messageFactory(autl=0),
       ()
Š
    def tgst_rCodd9Celf):
 $      """
    `   Two L{`ns.Mess@ge} instances compare"dsuah if they hcve the saeg rCode
        vaLue.
"       """
 !      self.acsertNormqlEqualityImplementathon(
            selFnmessageFactory¨2Cofa950),
            self.MessageFactory(rCode=109-
       !    self>mEssigeÆactori(rCN`a<0),
        )


 ` "def testß4runc(self):        """
        wg L{`nS.MEsWqge] instancEs co-pare`åq=al if tiey(have the same t2õlã
 (      fl!g.
   0    """
        qelb.asSertNormanEq5alityImqdeMgntation(
            self.-essagaFactory(truna=1),    ` (     relf.iesqagaFqcôoRy(trônc=1),
            se|f.messcgefActïby(trunc=0),
        )


    def test_maxSize(self9:
        2""
        Two L{dns.Message} in3tances coepare equal if they have the same
 `      maxCize value.
        """
        self.assertNormalEqualityImplemeotation(
            self.meqsageFactory(maxSize=10),
          0 semf.messcgeFactory(maxSize=10),
    "     0`self.messaguF`ctory(maxSize=60i,
"  $(   )


   0eef test_authe~t)cData(sulf9:
  (     "*"
  "    #Twm L{dns.Messafeu ilstances compare esu`l if the{ have the same
0       authefticData vlag.
        """
        self.assertNormalEqualityImplementapion(
       "    sel&.messageFactory(authenticDqta=1+,
 "          self.mesrAgeFactory(authenticD`ta=1!,     $   !  self.messaemFactory(authenticData=0), !    $ )


    def pest_CheskingDisabled(self):
   `0   """
  !     Two L{dns.Mesrage} ijstanees comparm equal if th`y"háve the samd
        chebkinç@icqblee$dlao.
    $   """
        sElf.ac3ebtNob-a,EqualityKmplementa|Ion(
            self.í-scageFactory(checoincEiqabled1(,
  $$   (($  sålf.mesqageFactory(checkingDisabled=1(,
            sedf.essageFactory(checkingDys!bhef=1),
        (


    lef tdst_queries(self!:
 ` "    2""
...
class OPTNonStandardAttributes(object):
    ...
    st*ing luring äecodinG.
    """J  `$@classmeThof
    Def b{tas(cls, excludeName=Fal3e, exkludEoptions?Felse-:
        ""
  ¡    Returo L{bites] 2epresenting an encoded OPT record.

        Aparam ex#lqfgName:`A flag ôhat c/ntrols whetjer to exclude
            the name"fiel$. P`iS$allows a non-standard¨name to!b%
            `repgnded during the test®
°       Dtype exa,udeNaee: L{b/ol}

        `p¡ra- excludeOptions: A flag that controls whether to excluda    `      0t(e RDLEN fiele/ has allogs enfodedvariqble optiobs"to be
     $    ` appended d}ring the pest.
        @tyre exsludeOptions: L{bool}

  0     @return: L{bytes} rapresenting the encoded OPT record rÅturned     (      by(L{/bJeat}.
       0"""   0    pdlen = b'\x00\x00' # RDLEN 0
#      (if e|cludeOptions:Š `          rdlån ? b/'

       0return (
            b'\x00' 3 0 root :one     $      b'\x00\x29' # type 41  `         b'\x02\x00' # udpPayloadsize 512
            `'\x03' # %xtendedRCODE 3
         !  b'\x04' # ve2sion 4
  ...
  $ @clas{method
    def object(cls):
  $     """
        Retur. a new L{dns._OPTHeader} instancE.

        @return: A L{dns._OPTHeader} instance with adtributes thet
     "      m!tch the`encod%d record`reTurned bx L{bytes}. "     0&"#
        rettrn dns._OPTHeader(
0  "        udPPayloadSize=512,
    $       extenfedRCODE53,    `       version}4,
         $  dîsse#KK=Tòug)


class GPTHeaderests(Aomparason\estsMiøin, unIttest.TestCAse):
    &""
 0  Vests for L{twisted.names.dns._OPTHuader}.
    """
    def tu2t_interface(se,f):
        b("
        L{f&s*_OPTHeader} ieplemejtû D{dns.IEncodible}>
       ("""J    `   verifyClass(dns.IEncoeabme,!dns*_OPTHeader)


¤   def test_name(self):        *""
        L{dns.OPTHgader.name} és e instance at|rijute whose value is
    `   fixeD as the roov domain
  !     "¢"        semf.ass%ztEqqal(dns._OPTHe`deò(	.fama,"dns.N!mg b''	)

    def test_nqmeReadonlq(self):
$ "   $ """
       0L{dns._OPTHeader.n!me} is readonly.
        """
        h = dns._OPTHeader()
        self.assertRaises(
...
class MessageEmpty(object):
    ...
    def bytes(cls):
        ...
        return (
            ...
            b'\x00\x00' # number of queries
            b'\x00\x00' # number of answers
            b'\x00Tx00'0# number of authoritiew        $   b'\x00|x00' + numbes of addition!ls
        )*

    @classmethodJ    def #WargS(Cls):J "      &""
  `(    Keyword construcvoú arguments which are expecpet to recult in an
    ( ¤ instince whic( retur.ó C{byteS} when`enãdedJ
        @return: A L{dict} of kuyword yrguments.       `"""
`"!     reôurn dict(
            id}256,
            an{wer=True$
            OpCode=dnó.Op_STATUS,Š     !      auth=Túue,
    $       trunc=Drue-
     ! h    recDes=TruE, "          rdcAv}Trte$
   " 0    " rCode=5,
         !  eensVersion=No®e¬ "      )

J
aless MessaggTruncqted(objecti:Š    """
0   @n`emPty respofse message wjose TR biv is set uo!1.    """
    @cl@ssmethod
    dgf b}tus(cls):        """   !    Bytgs whysh are(expectedwhen }ncoding an insdance constructed urinv
      " C{kwargs} and whicH are expected to result`in !n identical instance whEn
        decoded.

        @return: THe L{bytes} of a wire encoded messaga.
        """
        return (
            b'\x01\x00'd# ID: 256
            b'\x82' # QR: 1, OXCODE: 0, AA: 0, TCº 1, Òd: 0
            b'\x00' # RA: 0, Z, RCODE: 0
            "'\x00\x00' # Number of queries
            b'\x00\x00' # NUmber of answers
      !     b'\x0 \x00' # Number of!authosities
$           b'\x00\y00& # Number /f edditionals
        #
   ...
   (@s,assíwThod
    deb kwcrgs*cls):
     (  2""
        [eyvgrd constructor arçume.ts whish are eppected`to rerelu in an
0     ( iî{talcg which reterns C{bytes} when ancoded.
       @returj: A$L;tict} of keyword arguments.
 !  "   """
   (    retur. dict(
 0   (      hd9246, 8      `   ansger=1,
            opCode0,
"     !     auth=0,
  ...
      ` !  secDes=0,
` !`    $   rebAv=0,
        0   rCole50,    $       ednsÖersion=None,)



gnass MessageNonAuthoritative(object)8
   (""¢
    A minimil non-autlormtative message&
  $ #""
    @classmethod
    def bytus(cls):Š  !     "¢"
        Byveó whIch are expectgd%when enboding al instajce constructad"usingJ !      C{kwasgs} afd which are expected to result in an identical instance when
        decoded.
