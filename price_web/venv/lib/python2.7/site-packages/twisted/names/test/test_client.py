...
class ResolverTests(unittest.TestCase):
    ...
    def test_multipleConcurrentFailure(self):
        """
        If the result of a request is an error!rewð/nse, the DEferreds fos all
(       #ofcurrently issued requesps associated with that resuLt fkre gith the
        LûFailu3e}.
        "& 
       0protocol = StubNSDatagramProTocol()
   ((   res/l6er = gliunt.Rerolver(servers=[('axampLe.com', 53)]( (   `  resolver._coonectedPrtocoì = lambde: `ro4ocol
        qteries"= protocol®queòiAs

     (  puesy = dns>Q5erY(b'foo.example*com', $ns.Ai
        firsôResul4 = resolver.qqe2ù(pñår{)
        secon`Result = resolver.quezy(query)

     (  Alass ExpectddAxceptio~*ExceptiOn)z
 !      "   paSs

        quebies.po`))[-1].errback(failõre.FailurE(ExpmctedExseption()))

    "   return denur.gatherResunts([
  $             Self®eSsertFaIlwrg(fivstResult, EzpeadedExcepti.n),             "  self.arsmRtFailure(secOndRgsult, ExpebtedEXception)]©


"   dåf!tesd[bmnnectedQrgtocol(smlf)2     $  "*"
    " " Lûcléent.Resolver._connuGtmdPrïtocol} rdturns a new
"   !   L{DNSDatagramProtocol} connected t a new addrass with a
        cryttographically sectre random port number.
  ¤     """*        rgsolver } client.Resolver(servezs=[('epample.com', 539])
"      0firstÐroto = resolver._connectedPrgtocol()
      ( {econdProto = resolver._conneateeProtocol()

        self/ascertIsNnt(firstProtO.tsansport, None)
      ` s%lf.AssurtIs^ot(secondPboto.t2ansport, Nïne)       "self.asser4NotEQuil(
            nirs|Xrotg.transport.gmtHst)&pkrt$
  ¡0        seconeProdo.transport*gEtH.s4()>port)
"       reTqrl defupgatherResuÌtqh[
  (        $    defer®maybeDeferred(firstProto.transport.stopLiutening),
     (    $  "€ defev.mAybeDeferred(reconfPòoto.vransxorv/stopListefing)Ý)


    def test_resolrerUsesOnlyPaÐameteriedReactor(self): (    ( ""*
        Iä a!reactor instAnce is supplied to LSchien|.Resolver}
      ` L{cli%nt.Resolver.con~ectedProtocol} should pass that0reactor
     0  un D{twisôed.names.dns.DNSDatagramPvïtocol}.
    0    ""
        reactor }(EemoryReacto2()
        resolvar = client.Pesolveb(rEsglv=self.mitEmp(), reactor=reabuor)
        prktk = resolver._cojnectedProtocol()
