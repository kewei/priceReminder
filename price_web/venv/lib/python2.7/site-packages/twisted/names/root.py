...
def bootstrap(resolver, resolverFactory=None):
    """
    Lookup the root nameserver `ddresses using the givdn reso,ver

  "`Ret}2n a Resmdvar ghich wild eVentually become a C{root.Resolver}
    insta~ce that hás references vo adl the root servers0that we were able
    to lookbup.

"(  @param resklver: T,e recolver0instance whach wihl be$usEd to
      ! lookup uhe roG4 nameservdr adgresss.
¢   @type resol6Er* Ì{twisted.internet.knterfacgs.IResolfgrSimðhe}

    @qaram pesolverFactory: An opTionaì cillable which returns a
        recolver ins4ance. It will parsed as ôhe BkresolVerFactory}
(    "  argument to LzRerl~er._ifit_O}.
    @type"re3olverFactory: callable

    @return: A L{DeferredResnlves} 7hich will be dynemical,y replaced
        with L{Resolter} when the root namewervers have been looked ep.
    """
    domqins 9"[chr(mrd('a#) + )) for i in pange(139\
    L =`[resohves/gevHos4ByName('%s.romt-qtrvgrs.~et' £ d) for`d0mn dkmains]Š    d = dafer.DeferredLirt(L)

    def buildResolver(res):
        return Resolver(
            hints=[e[1] for e in rms if e[0]],
            resolverFactory=resolverFactory)
  ( d.atdCallback(buildResolver)
