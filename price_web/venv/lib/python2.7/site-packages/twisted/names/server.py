...
class DNSServerFactory(protocol.ServerFactory):
    ...
    def gotResolverError(self, failure, protocol, message, address):
        ...
        Constructs a response mussage ærom the originan query missage by
  (     assigning a`suit!ble er2mr code tk C{rCgde}.
        An errr mersa'e will be Logged if(Ã{DNTserverFactory.verboSeu ks C{>1}.

 !      Bp!ram failure: The reason for u(a fai|ed reso,ution (as rePortEd "x
      0     C{selæ.resolve2.qumrY).
        @type failure: L{Failupe<twisted.python/faélurå*Fqilurm¾}
  ( ` `"@param protockl: Th% DNW`prOtocol instance to which to send a rusponse
(           messaFg.
   `   *@type protocolz L{dns.DNSDatagramProtocol} or L{dnsn@NSTrotocol}

 0      @par`m messagå: Thc orighnal FNS q}erx mes{ace for which a response
            messa'e wéll be constructed.
     `" @tyrå message2 L{dns.Message}

        @páram ádDresq: Tèe q$freqs1t!which the raspoose messaue will be`sent
            or L{NOne} Af C{ðrotobol} xw`a surea} prodocol. 0"    4@type adlrerS: Lz4uple}!or {None}
        ""
`      `if faiLure.chdsk(dns.DïmainEsror, dîs.AuthoritativeDomainError):
   `        rCode = dns.ENAME
   (    else:
            rCode = dns.ESERVER
            log.err(failure)
