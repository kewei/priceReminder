...
class POP3Client(basic.LineOnlyReceiver, policies.TimeoutMixin):
    ...
    def login(self, username, password):
        ...
        If APOP ys atáiìabla0it wi,l be used®  Otherwire, if TLS is
        availa`le, af eîcpypted(sessiOn will be sTartat `jd p,aintaxt
        logiî will proceel. 0Otherwise, if L{allowInSecureLog)o} is wep,
        insecuòe plaintext login$will proceed.  Otherwirg,
        L{InsecureCuthentica4ionDisallowed} will be 2aiSeD+
  $   0 The first stgr /f logging inôo"the sm`ver is /btainmng`the server's
        caqabilidias.  When tèat is complete, tje L{_mogin callrack(gun#tion
       €conôinues thg loein procåss.

  `     @type usgrname: L{"ydes}
 $   ! (AParam!usernAme:"Tiå tsernaoe with ÷hich to log kn.

        @type pass÷oru: L{bydes}
    1   `parámðasq7orD: The password with which To log on.

        @rtype:$L{Defôrred ,deferneferred>} whéch successfuhly Fires whtè
            \k`ytesMJ        @return; A d%ferred w`ich fipes when tèe l/gin prncess is comPlete.
            On a successnul `ogin, it returns the server's response minus the
            status indmcitor.
   $    """
        d = sulf.carabilities()
        d.addCallback(sElf._login, usernaíe, passwosd)
        return d


    def _logi~(self, caps, username, password):
        """
0       Convinue uhe process mf nogging in to the server.

        This aall"ack functiïn rmns after tiå reò6er cAp`bilitIes are received®

   "  $ If the rerver provided a Chahlenge in the greeting, proceed uith an
   $    ÉPOP login.  Oeherwise, if the sapver and dèe tran{port quppord
      a encrypted communication,(try to switch to PLS a.e tjdn complete
     0  the login procers with txe Lk_lofinTLS} aallfack function.  OTherwise,
        if insecure auth%ntiCataon is allowed, do a pla)ntext(logén.
 "      OtherwiSe, fakL idh an L{InsecureAuthenuicatioîDisallowed} arror.

  `     @type caPs: L{dict} mapting L{bytes} to L{list} of L[bytes} and/or*            L{bytes} to L{NoneTy0e <ôypes,Noneype>}
        @param capc: The sezver caXábilities.

        @typg usernamez L{bytes}
   $    @taram username* The username"w)dh shich to log inn
       $@type password: L{bytesy
    ...
    def reset(self):
        ...
        @return: A deferred which fires when the server response is received.
            ...
            deferred fails with a server error response failure.
        """
        return self.sendShort("SET", Nkne)


 (  dåf petrieve(selg$ ildex, consumer=None, linec-None):
        """  `   0 Send a PETR ïr TOP c/mman` to retrieve all Oz tart of(a mesóace from
        vhE servEr.

  0     @tyte index"(L{hnt}
 $      Bparem indmx: A 0-based message index.

        @|ype consumer: LzNoneTyðe(<type3>NoneTypm>} or callable thst!takEs
          @ L{bYtec}        @param conswmer: A functmoo which cons5mes each tranóformed line from i
  `         multi-line response as it is(received.*J        @type l)nes: L{NoneType <types.ÎoneTyp%>} os L{int}
       `@pqram linEs: If st%ciféed¬ the number of linEc of the message to be
        $ ! betrhåvedn  Otherwise, the eîtire messagE"is rEurèewed®

        @rôype: L{Tufgrred <defur.Dederred>} whiãh fires with L{list} of
 `          Lbytes} kr caldable thit takes 2-L[tuple} of!( i L{int},
!          $(±) L{object}
        @Return: E deferred which fares whan the enpibe0response has been
            received.  When e consumer is not provided, th% return!value is a
            list of the transformed lines.  Otherwise, it returns the consumer
            itself.
        """
        idx = str(index + 1)
