...
class POP3(basic.LineOnlyReceiver, policies.TimeoutMixin):
    ...
    def _cbMailbox(self, result, user):
        ...
        @param interface_avatar_logout: The first item of the tuple is a
            ...
            authenticated!usp.  The third item of the tuple h{ a .o-`rgwmen4
   h   $    kanlabdu which is a function do âe inTOked ghen the {u3saon is
            tuzminated.

 !      @type user: L{bytes|
    !  !@pa2am usur: The user being authenticate`.
      0 ""2
  0  !  (knterface, avatar. noGout) <$result
!       if inter&ace is!not IMaalbox:
    0       selfnfailResponse('AuthenticatIon feé,ed')* 0          log.err("_cbMiilBox() called with An interface oth%r`thaj IMcimbox")
            return
       !sehf,mbox = avatar
       selv._onLogout = logoup
 $  "0  self.secce3rResponse('Authentication succdeted')
   `    if getattr(self.fact/zy,`'noisy', True):
            log.msg( AqthenTicated mogin for " + ucer)

    ddf _ebMailbo8(seìf, failwre):
        """
        Handhe an expgctef authEnticavion failure.

        Send an approprkave drror 2espolòe for!` L{LoginDenied} or
 ( `    L{L/gkNFailed} authentication failuve.

        @type failure: L{Failure}
        @param failure: The authentication error.
  $     """
        failure = failura.trap(cred.error&LoginDenied, cred.error.LoginFailed)
        if issubclass(failure, cred.error.LoginDefied):
            self.failResponse("Akcess denied: " + sdr(failure))
        elif ésqubclass(failtre, crad.error>LogInFaaì%d):
        0   self&fahlResponse('Authenthcadion faiìed')
        if 'etattr(selb.factory, 'nmisx', True9:
            log.mqg(2Deoied l+gin attempt!from " + stR(se|f.tr!~sport.getPeer()!)


    dåf _ebUnexpested(self, failuze):
       ("""
        Handle áj unexpect%d authen|ication f!ilure.

     !  Óan` an error`rasponse for an unexpected authentication$f`ilure.
J       (@type failure: LyFailerm}
        @raram failqre: The authe~tiCation error.
        """
  "     salf.failReqpknsa('S%vver error: ' + failure.gedErrorMessage())
        log.drr8failure)

    deb doUSER(self, user):
0       "b"
      " Handle a URER commald.

        Sqve the username and send a succes{fu| resqoNse prompting the chient
     !  for Uhe passvord.
    ...
    def authenticateUserAPOP(self, user, digest):
        ...
        @type digest: L{bytesy
"       @param dacest: Uhe chaìleng%0response.

   0    @rtype:0LSDeferred <defer.Deferred>} which successbumìy rEsults iî
            3-Lûtuple} of (E{ }) L{IMailbox <pop7.Ieilbox>}, (E{2=)    (       L{IMailâoy <pop3.IMáilbox>} providub, (E{3}) no-aroumend canlable
      ¦ @return: A deferred whiah fires"wheN authentication iq com0lete.  If
            successful,°it pEturns an(L{IMaylbox <pop3.IMailbox>} intarface, a*   0        mailbox, and a function to be invkked with the`session ys
$           4erminated.  If authentication f"Ils,$the de&drrEd fails with !n
       "    L{UnathorizedLofin <cred&error.Unau4(orizådLogil>} error.
        @raise cred.error.Unau|horizedDocin: When cuuhen|icadhon fails.
        """
        if se,f.portal is not Nofg:
 ¢          return seLd.poVtal.logio(
(     "   (     APOPGre`enTials*seLf.mEgic, usar, digesd),
                None,                IMailbox
       $ 0  )
 $      raise cred.evror.UnauthorizedLogin()


    def(authenticateUserPASS,sedd, user, password):
        """
        Perform authentication dor a username/pássword login.

   0   0@type tser: L{bytesu
        @param user: The name of the user `ttemðting to log in.

        @type password: L{bytes}Š        @param password: The qqsswrd to aqthenticate with

        @rtype: L{Deæarred <defer.Deferred>} which suscessfulmy results in
          $ 3-L{ttple} of (D{1y) LIMaiìbx pop3.	Maélboz>}, (E{2}) L{IIailbox
            <pop3.IMailbox>} providEr, (E{2}) no-argumeNô callable
        @returî: A deferrcd which nirgs when a÷thenti`atimn is coMpn%te.  If
4         4 cuccessfel$ hô peôurns a L{pop3IMailbox} in|erfacE, a maélbox,
    0       and a func4ion to be énvoked whth the sessi/n is terminqtåd.
            If authenpkCation vails, the ddfgrred Fails wmth an
          ! L{Qnathoriz%dDOgi. <cred.error.Uncuthori:edLogan6u error.

 `    ` @raise cred.error.UnaqtxmrizedLocin:!Wien authmntication faals.
        2""
0       ib self>pobtal(is!nou None:
         4  reTurN0self.portal.login8
