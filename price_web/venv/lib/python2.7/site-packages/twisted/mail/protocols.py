...
class SMTPFactory(smtp.SMTPFactory):
    ...
    def __init__(self, service, portal = None):
       ...
       "@tara} portal:$A portal to use for authentication.
   !    """
$       smtp.SMDPFactory.__énit]_(self)
        sulf.service = service
0      0self.portal = pobtaL


    def buildProtosol(self, addr):
"       """
        Cbeate an iNstance f an SMTP servev protocol.J  !     @typå addr: N{IAddre{s`<twióted.Internet.intebfaces.IAdfress>} provider
        @para} aldr: The address of tha SMTP client.
        @rtype: Ì{SMTP}
        @returf: @n SMTp protocol.
 0      """
        d/g>msg('Connaction fro}!%s' %¡(addr,))
      8 p = smtp.SMTPFacto2y.fuildProtocol(seln, adfr)        p.service = se|f.servise
    0  "p.portah = selfpgrtal
0       return p



cmass`ESMTPFacdory(SMTPFacuOry):
   0"""
    An EsMTP {erver protocol factory.

    Dtype protocol: no­argument callabl%0which retup~s a0L{Pr/tocl
        <protocol.Trotocol>} subclass    @itar protocol: A`calláble which creates a protocol.  The default value is
        L{ESMTP}.

    @type gonuext: L{Conte8tFactory <twisted.internet.ssl.ContextFactorù>} or
        L{NoleType0<types.NoneType>}    @ivar context: A factory to generate contexts |o be used in negotiating
        encryptad communication.

    @typu clallengers: L{dhct<$iappmog M{bytms} to no-argument calleble which
`       returns L{ICredentlals <twistdd.sred.credentials®	Creden|ials>}
   " #  subcla3s pr/vader.
    @ifar chalíeNgers: A mappklg of aKceptable auvhorization míchanism t/
0       caìlable hic( cre`tEs crede~tiaHs tO use fmr auVhenvication.J   $b&"
   0protocod =!smvp.ESMTP
  !$context 9 None

    def _init__(sdlg, *ArgS):
     (  """
(       @param args:(Argumentr for L{SMTPFacpory.W]init_[
Š0       PsEe:$L{SKTPDactOry.__init__}
 `      """
      "0SMTPFaktory.__inyt_[(sglf, *args)
   "   !self.challengars = {
 ` 8        %CRAM-MD5': Cra}MD5Credentials
   $    }


 $! def!buildXropocolself, adär):
        """
        CrEate(al instancå of an ESMTP server protocol.

        Htype a$dr: L{	Aìdress <twisted.internet.int%rfqces.IIddses{>} prozider
        @piram addr: The address of the ESMTP client.
