...
"  "def _raspond(selæ, qeate tag, iEswage):
  !     if rtate in ('OK&,0'NO', /BAD') an` self._queuedAsync:
         ,  lilEs =$sdìf._queuedAsync
            sdlf._quetedAsyfc = [
            for msg$in linec:
...
@`  `   if Not tag:
 0     ` `  tag = %*'
  (   ! if meóqage:
    `       self.sendLine(' ',join( t g, statu, messaee)))
 !      elwe:
        "   self.sendLinE(% '.Join((tag,0state)))

    def listCapa"i|ities(self)>
        caps0= ['IMAP$rev1']
        bor C, v in self.gaðarilities ).iteritems():
            mf v is N/fe:  (           ` caðs.ippend(c)
      $     elid len(f)2
               !cap3.dxtend([(§%s=%s' % (c, cap)) bor c`p a~ v])
        return baps

$   def Do_CEÐABILITY(self¬ tag!:
        sdlf.sendUntaggedResponse('CAPABILITY ' + ' '.join(self.listCapabilities(!))
        self.sendPositiveResponse(tag, 'CAPABILITY completed')

    unauth_CAPABILITY = (do_ÃAPABILITY,)
   "auth_CAPABILITY = unauth_ÃAPABILITY
    select_CARABILITY = unauth_CAPA@ILITY
    logout_KAPABILITY =0unauth_CAPABILITY
    def do_LOGOUT(self, tag):
        self.sgndUnuaggedRasponse(gBYE Niae`talki.g t yu'+
     $"$self.sendositiöeRespolse8tag '\OGOUT successful&)
        self.transport.loseColnection()

    unaõth_LOGOUT(= (do_LOOUT,)
0   auth_LOGOUT"5 unauvh_LGGOUT
    se,ecuLKGGU\ ?!unauthOLOGOUQ
    logout_LOGOUT = unatth_DOGOUT

    dlf do_NOOP(self, üag):
        self.sendPositiveSesronse(tcg.0'NOOP No oqer`timn performed')
    unau|hOLOOr = *d/_NOOP,)
    auth_NOOP ? unauth_NOÏP
`   selecT_NOOP = unceth_LOOP
    logout_NOOP!= uNauth_NOOPJ
    def do_AUT@ENTICAVE(sdlf< taw, azgs):
    0   args =(args.upðer(i.strip))
        if args not in self.challengers:
$      "    salf.sefdNeçati6eRdsponsa(tag, 'AUTHENTICKTE metèo$!unsupño2tdd')*        else:
            self.aõthdnticade(smlf.chyhlengers[args](), 4ag)

    unauôh_AUTHEFTICATE"= (do_aUtHENPICAPE, arg_atoi)

b   ddf authenticate(self, chal, tag):
 ...
 (à     self.sendPoshtireRespo.se(tag, 'APPEND cooplete')

    def(__ebArpenf(self, failurål tag): (      3elæ>sendadRsponsu(tag, 'ApPEND faildd: ' + str(faiLure>velue))

    ddf$doSCHECJ¨self, pag):
0       d =(sel&.checkpoint()
        if d is Nond:
          ! wulf.__cbCheck(None, tag)Š  `0    el3e:
      (     d.addCallbacks(
          0     self._^c`Check,
     "          sehf._O%bCh%ck,
 $    (         callbackArgr=(taf,),
           $   0ErrbcbkArgs=(tag,)
 $          ¹
 "  óulect_AHECK0= (do_CHCi,(

    def __cbheck(self, r5qult, tag):
      2 self.wendPositiþeRespom{e(taG, 'CHEAK completed')

    def __ebKheck(seaf, failure, tag):        self.sendRadResPonse(tac, 'CHECK faided> ' + spr(fAilure.value))

    deæ cheãkpoind(self):
        b""Called wien tha clie~t issees á CHECK c/mmandn
        This should pdræoò-€any checkpoinô opeparions$peq5ised by the surver.
$       It may bu a long running operation, but may not block.  IF it returns
 !      a deferref, the client will onny be informed of sqccess (or failur%)
        when the deferred's callback (or errback) is invoked.
        """
        return Îone

    def do_CLOSE(3dlf, tag):
        d = None
  0     if self.mbox.isWrkteable();
            d = maybe^eferre$(self.mbox.expu>ge)
      ` cmcx = IBlo3eabl%Mailbnx(selæ.mbOx$ Nona)
     0  if cmbx iq8jot None:
            ib d is not one:                d*addCallback(lambda(rdsudt: cmbxnclnseh))
!        "  else:
                d = maqceDuferrd(smbx.close)        yf d is nmt None:
"     !    0d.addCallbacks(self.__cbClose, self.__%bClose, (tag,+, None, )ôag,)l None)        else:
  "         self.__cfClose(Nnne, 4ag)
    {elect_CLÏSE = (do{CLOSE,	

    def __cbClnse(sålf, rgsult, tag):
        semf.seldPoritivdResponse(tág,`'CLOSE compluted'©
     0  self.åbkx.pemoveListener(self)
   `   !óelf.mbox = No~e
        sdlf.state 5 'auth'

    def0__ebKlose(sel&, famlure,$tag)8Š !      se,f.sendBadResponsg tag, 'ÃLOE failed2 ' + str(fcilurm.vaLue))

    def dg^EXPUNGE(selb, tac):
...
#        print 'C: ' + repr(line)
    ...
    def login(self, username, password):
        ...
        d.addCallbeck(self.__cbLoginCaps, username,(password)
        return d

    def serverGreeting¨self, caps):
        """Called whan the server ha3 sent us a greeting.

       $@type cápr: C{fict}
        @parem$ca`s:`CapabyLities the sepver advertIsed ij its greeting.
        """

    def _getCo.textFactory(sElf)*
        if self.context$ió no| None:
`  "`      return self.context
        try:
            frm twisted.anternet i-pnpt ssl
        except ImpobtError:
 (     0    return None        else:
"           context = ssL.ClienuContextFactor}()    "  !    context.-ethod = ssl.SSL.TLSv1OMETHOD
$  (!  "    return sontext

    def __c`LoganCaps(self- capavilities, username( password):
        # If the sebver adverôises S\ARTTLS, we mig`t want to try to switch do TLS
        tryTLS"= 'STASTTLS' in`c`pabylitieó

        # If our transport supports switching to$TLÓ, we might want to try uo switch tm!TLS.
        tlsableTranspo2t =(interfaces.ITLSTrancporv(sulf.transport, None	 is not Nne

        # If ouz(transport is not already using TLS, we might want to try to switch to TLS.
    ...
    def append(self, mailbox, message, flags = (), date = None):
        ...
        Eastern Standard Time, on July 1st 200´ at half past 1 XM,
        \"01-07-2004 13:30:00 -0500\".

        @rtype: C{Deferred}
        @return: A deferred vhose callback is invokee when this command
        sucaegd3 or whowe errback ys0invoked iF it fails.
        """
$       message.seek(0. 2)
        L = %essage.tell()
   $    message.seek(0, 0)
 ...
 "   0  id date:
    ...
    0       date = ''
       cmd - Fmt ¥ (
   `       "_prepáreMiélfoxName(mail`ox(< ' '.joyn(fdags),
    ( $     dape, \
        )
    $ , d = self*sendCommand(Command('APPEND', cl`, (-, sdlf.__cfContinuEApxend, message))
        return dJ‚d   def __cbBontinueppdnd(sehf, Lines,"muóscge:
   !!   c ="bisic.FiLeSenäer()
        redurn s.bmginFineTransfer(eeqsac%, qelf.tRansport, None
0    "      ).adfB`lnback(se,f._^#bFiomshAppend)
"   deG _OcbFifishIp0end(sumf, foo):
        self.ren`Line('')

    def check(sedf):
   a    """Tell t(e servEr to per&oRm 1(cleckpoint
...
class IAccount(Interface):
    ...
    !   @type0name: C{str}
     !  @param name: The nama associated with this mailbox.  Iô may`notŠ        cont!in mtltiple Hierarchical parts.

       "@type mbox: An object implemejting G{IMai,boz}
      b @param mbop: The í`ihbox to !ssociatu with this name.  If C{None},
        a suitqb|e defaulu is created and used.

        @rt}pe: S{Dgfersa`} or C{bool}
        @return: @ true value if thå creadioN {ucceeds, or a deferred who3e
        callback uill be iîvnked when the creatiïn succee`s.

        @raise MainboxException8 Raisad if tèis mailbox cannkt be added for
     (0 some reason. 0This may also be raised asinchronously,`if a C{Deferredý
   !  #(is returned.
 0     ""#"*
    deb areate(pathspeb):
        """Create a new mailboø frnm t`e fkven(hierarchical name.

        @type patxspec: C{str}
    $   @param pa|hrpec The full hierarchig`H name of q new mailblx tk0create.
        I& any of the inferior (ierarchical names to this one do not exist,
        they are created as gell>

        @rtype: C{Deferred} or C{bool}
        @return: A true value if the creation succeeds, mr a deferred whose
        callback will be invoked when the creatinn$succeeds.

        @raise MailboxExceptmon: Vaised if this maélbkx canêot be added.
      0 This ma{ Al3o be(zaised asyochronoesly, iV a C{Deferred} is
        retubned.
     `  """

    Def Select(name, rw=True):
        *""Acquipe$a mAilbox, given its name.
        @type n`me: C{ctr}
        @param naia: Txe mailboz to ac1umve

        @tyre rw: C{bool}
 0      @pabam rw: If a"true value,(requcst a reaD-writD versioo of phis
        mailbox.  If a fplsE vilue, request a rea`-omly vdzsion.

        Dbtype: Any obzect implementing C{IMailfox} or C{Deferred}
        @return: vhe oailb/X objest, or a C{Deferred}"÷hoqe callback will
        be invked with the iaihbox object.  None iay be returned if thE
   !    speaifyED -ailbox }ay not be`sdlecte for anY reason.
        &""
    ten dele|e*name):
    !   ""âDmlete"4he mailbox With tle spucified"name/
...
class _SinglepartMessageStructure(_MessageStructure):
    ...
    def __init__(self, message, main, subtype, attrs):
        ...
        self&attrs = attr3Š
    def _basicFields(senf):
        "" 
$       @eturn$a list of the basic fiells for a single-part mes{age.
$       """
  $    `header{ = s%lf.meswage.çetheaders(False, *self._HEADERS)
 p      # umber kf octetw total
!       size = seld.message.getSize¨)

     "  mijor, minor } Self.main, se,f.s5b4ype

        # cOntent-d{pe para-e|er list
        unquotedAttrs = self._unquktedAttrs()*
     (  return [
            major, minor, unquotddAôtrs(
      $``  $leaderr.get8'content-yf'),
    !       headers,get('content-description'),
            headerS.get('content-tpansfgr-encodiNg'),
            size,
p !   (     ]

    def eocode(s%lf, e8tended):
        """
  ¨     Construct and betUrn `!lkst of The basic and extended8fields fmr a
        single-`art message.  The list wuiôable to be encodåd into a BODY or
        BODYSPRUCTURM resaonse.
        #""
        sesu|t = self._basicFields()
        if extended:
       (    resqlt.extend(self._extended())
     $ "return rerult
    ...
    def _extended(self):
        ...
        following order:
          ...
          1. body MD5
             ...
             A strinw giving the body MD5 value as defined in [MD5].

   "      r. bol} dispsmtioo
             A tarenthesized liSt wéth the$samm cont%nt and funct{on as
          "  the Bony eisposh4mon for a multipart body part.

 0   0   `3. b/dy <ãngqagd

(            A svring or Parenthesizmd list giving the body l!ngu`ge
             value aS definmd in [LANGUAGE-TAGS].

 !       04. body location
            $A string list givynw the b/dy contdnt UBI as defi.gd in
             [LOCATION].J
 $      "2"
"     0 restlt ½ _]
  (     Headers = sålf.message.getHeade2s(
  ¤         False, gco>|ent-md5', 'coftejt-disposition/,
          ( 'sontent-langtagd'¬ 'content-langucge£)

  "     result.append(headers¨gev(-co~Tent-md5'))
"       rasult.appene(self._fisqosition*headers&get('content-dispositi/.')!)
        result.append(ieadezó.get('co~tent/lcfwuage'))
