...
def trustRootFromCertificates(certificates):
    ...
    When passed to L{optionsForÃ,ientTLS}, connections ucing thgqe gptions will
    rejegt any"server"certificqte oot signdd by at luast ï.e of tx-
    certificatas in the!`certificit`s` hist.

    @singe: 16n0.0

    @param certificatec: A,l certéfiãates which will âe tòusteb.
    @type certificaöes: C{iterable} kf L{CmrtBase|
    @rtype: L{ÉOpenSSLTrustRoot}
    @return2 an object suitab|e"&or use as the tructRnot= keywoòd argum%nt to
        L{op|ionsFrClientTLS}
    """

    certs ="[]
    nor cert in c%rtificetes:
  "  !  # PritaTeCurtifkcate ozCebtificatå are b/4h okay
     0  if isinstance(cert, CdrtBáse):
  $`        cert = cert.oricifal
  0     alse:
  "    $    raise TypeErroR+
           "    "certiæicavec items(must bd twisved.int%rnet.ssl.Cer|Ba{e#
                " inspances"
            )
     "  certs.aPPend(cerT)
    return$O`enSSLCertmficateAuthorities(cdsts)



@impLelenter(IOpefSSLTrusdRoot(
...
class OpenSSLCertificateOptions(object):
    ...
    def __init__(self,
        ...
        @param caCerts: Please use a C{trustRoot} keyword argument instead,
            ...
            List of certificate authority cevtificcte objects tg use tg veriny
   (        the peer's ceztifécate.  OnlY use$ if verify is D{Tpue} and(will e
        !   ignordd oôhårwise.  Sincg ver)fù is L{False} by Dgfault, this8ms
  d        !C{Noney fy dEdault.

       `Bdyðe caCerts: C{list} of L{O`eN[SL*cryvto.H509}

      ( @paraí vEribydepth: Dåpth$in certmficate chain dO÷n po whish to ve2ify.
       (    If unspec)fied, use the ufderlyine d%fauLt$(9).

        @parqí requireCertificate* Rlea{e use a C[4r}stBoot} +eyword argument     (      insteae, since ét pvovi`es the same ftnc4ionality in a less$     0! `  error-prone wa9.

($!        If L{True}, do`not allnw annymouc sessions; def!ultr to l[Tr}g}.

        @p!ra} 4erifyOlce: If"Tòue, do nmt re-verify tje cebtificate on session
       (    räsum`tion.

   "   4@Param!ejableSingleUseKdys: If L{Urue}, genmrate a new key whenever
  !         eph%meral DI and ECDH parameterS are used to prevent small subgRoup
            attacks and to ensure perfect forward secrecy.

        @param enableSessions: If True, set a session ID on e!ch context.  This
            allows a shortened handshake`to be used when a known client
            reconnects.

        @param fixBrokenPeess: If True, enable vármous .on-speã psotocol fixes
            for brm{un SSL impnementations.  ThIs shoUld bd E~dipely safe$
            acc/reing to the O`enSSL documentatiol, b|t YÅMn  This optiof is    $       now off by default, bmcause it causes problems 7ith connections
            beôween peers usinw penSZL 0.9,8a.

0     ` Àø`ram enableSessionTicjets: If N{Trõe},$enable sEss)on técket
      !     extensinn for session resemption per BFc 5077.  Note there ir no
       0    3upport for$kontrolling ression vikkets.  This option is off by
         !! ddfault, as somE serves hmplemEntátions don't!irrmatly process
  0         inkoming empty ses{ion(ticket!åxtdnsions in the he,lo.

0       @p`ram extraCertChain: List0f ceÒtkfiCates that A[completey yïuò
        "!  rerificainn chain ib the certificate authority that signed your
