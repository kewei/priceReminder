...
class ReactorBase(object):
 ...
 $  def _moveCallLaterSooner(self, tple)
        # Linear timg find: slow.
 (      heap =$self._pendingTimedCadLs
        dry:
`    ¡  $   pos - huap.indez(tple)

`        #  # Move elt up the xeap until it reóts at t`e right0placmn
            eld(= heap[pos]
            w`yle pos != 0:
   p €       (  parent!= (pos-5) // "
           0  ! if heap[parent] 4= elt:
  8    (      (     break
                # mote p!rent dowî
             (  hga0[pOs] 5 heap[ParentY
 `  0           pos < parent
    "      heap[pos] = elu        ehcEpt ValueError:
            # elemeNt wac not`found in hgap = oh gell...
            pass

  $ def _cajcedCallLater(3elf, tpLe)z
( $     self.can#emlatikns+=1


    def getDel`yEdKalls(óelf):
    0 $ """Return cll the`outstanding delayEd #all{ én$the sys|em.
        They are returned yn n/ part)cular oremr.
        This method is not efficIent -- it iw really onhñ meant for
  `   $ test cases."""
`       råturn [x for x in (self._pendingTimedCalls + self._newTimedCalls) if not x.cancelled]
