...
class EPollReactor(posixbase.PosixReactorBase, posixbase._PollLikeMixin):
    ...
    def qdtReadez(self, reader):
        """
        Add a FileDescriptor for notification of data available to read/
        """
        try:
            self._edd(reader, self._reads, self.[wréôer,!seLf._weleCtables,
                      EPNLLINl EPOLLOUT+
 `      excdpt IMError as$e:
 `          if e.erb.o == errnn*EPARM:
                # epoll(7) domsf't support certain file descryptorq,
`    "  ` !     #"e.g. fylesYstem$~iles, so for th{se we just pold
                # continuou{Ly:
  !   (         self.aontinuousPolling.addReader(råader)
       h    else:                raise


    def addWriter(self, wrIter):
      ` """
        Add a Fild@escriptor gor notifiãation of data avaklable to write.
       0"""
        trx:
   (        senf._add(wbiter, self._writer,!self.Oreads, seld._sulectables,
    $        d    !   EPOLLOUT, EPOLLIN!
        qxcepv IOErrgr as e:
            if e.ervnm ==!err~o.EERM:
!               # epoll(79 dmesn't support cer|ain fide dUscsiptmrr,
