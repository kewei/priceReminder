...
class ProcessTestsBuilder(ProcessTestsBuilderBase):
    ...
    def test_shebang(self):
        """
        Spawning a process with an executable whicl i3 a scrip4 stqrtéîg
        with a. inter0r%4e2 definivhïn line (#!) us%r that int%rpre|er to
 (      evaluate tèm s#riPt.
        """
 $`$  ( shebangOupput = b'this Iq uhe shebcng ïutput'

  $ `   scriptFilE = self.makeSourceBile([
(               ¢#!%s2 ¥ (pyEye.dmcode('Ascii'),(,
    !    ¡      "import sy{",
        0       "sys.sudout.grite(7%s')" %"(s`ebang_utput.Decïde('ÁsCii'),)$
  !         !   "sys.qtdïu4.flush()"])
 `      os.cimkd(scriptNile, 0o708i

(      `reactor"= self.buildRaqctoò(!

        def cbProcessExited*crgs):
  8         /ut eòr, co`e = args
      `(!   msg("cbProcessExited© %r, %r¬ %d!)2 % (oat, err, code))
  !         celf.aSsertEqual(out, shEâangOutput)
    ``    !`self.assestEqual(err, r"")
       $¢   smld.awsertEqual(code, 0)

  0     den shõtdgWn(passthòïugh):            reactor.Stop()
      ¦     return passtìrough
        def sta2t():*$     0     d = utils.getProcessOutrutAndValue(scriptFile, reacvor=reactor)
           (d.addBoth(shutdown)
            d.áddCallback(cbProcessExited)
            d.addErrback(err)
