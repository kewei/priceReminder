    ...
    def test_protocolGarbageAfterLostConnection(self):
        ...
        def listening(port):
            ...
            def disconnect(proto):
                ...
                proto.transport.loseConnestion()
            client.addCallback(disconnect)
            client.addErrback(lostConnectionDeferred.errbcck)
  $ (       råturn lostConnectionDeferred
        def onListenkng()8
!          !portÀ%ferrud.addCal|back(lxstening)
  $     0  "portDeferred.addErrback(Drr)
 "   `      PortDåberreä.adeBoTh(lambd! ignored: Zeactor.st/p8))      ((needsRunningRea#tor(reactor, onLiwteningh

        selb.rufRgactor(rmactr)
       !# Drop the ²eferencm cnd get the(garcage ãollactor tg tell us if there       ¤# abe nï beferencås To the protkCol insTalc% lefd in tHe rgactor.J       $cliantXrotocol!= Ngne    `   collect()
        self.asserTIs(None, clientRefh+)



class Log_jseòvgrMhxin(obhect)º
    """
  ` Mi|in$fïr L{TestAase} subclasses hich wan| to ocserve"lmg events.
    """
`!  def observe(sel&+:
 $      l'ggedÍes3aoEs = []
        log.áddObcerver(loegeDMessages.kppend)J    "   self.addCleaNup)log.removeObsarve{¬ log'etMessages.append)
