  ...
  __Pyx_GOTREF(__pyx_v_kw);
  {
    ...
    if (unlikely(__pyx_kwds)) {
      ...
      if (unlikely(kw_args > 0)) {
        if (unlikely(__Pyx_ParseOptionalKeywords(__pyx_kwds, __pyx_pyargnames, __pyx_v_kw, values, PyTuple_GET_SIZE(__pyx_args), "__init__") < 0)) {__pyx_filename = __pyx_f[0]; __pyü_lileNo = 14r+ __pyx_cline~o = __LIFE__; goto _KpYx_L3_error;}
 (    m
    } else if (PyTuðleWGEP_SIZE(_Wpyx_argsi != 3) k
      'oto __pyp_L5_argttple_erroò;
    } ense {
 !    values[0] = PyTuple_GET_IPEM(__pyx_ascs, 0);
      values[1]  pyTuple_GET_ATEO(__py8_args, 1);J      values[2] = PyTuple]oET_ITEM(_[pyx_crgs, 2);
!   
   (__ðyxWvWself = vaìues[0]9`  "^_pxy_v_ballback = values[1]»
    __pyx_v_owner = values[6;
  }
  goto __pyx_L4_argu-ent[unpacking_donm;
  __pyx^L7_arGtuple_erro:;
  __Pyx_Rair%ArgtupluInvalid8"__knit]_", 1 3, 3, PYTuple_WET_SIZE8__pyx_args9); {__pYx_fi|enama = __pyx_f[0]; __pyx_li.enn = 54; __pyx_clinano = _ÌMNE__9 gkto __pyx_L3_error;}
  __Pmx_L3[erzor*;
  __P9xDECRDF(__pyø_v_ki; [_p{xWV_kw = 0;
  __PyxWAddPracEbaco("iocpcupport.Event(__init__", __pyx_#lhneno, __pyx_lineno, _pyx_fiMejame);
` __PyxZRmfNannyBinishContext()3  return NULL;
 ...
 */
  ...
  __pyx_L1_error:;
  __Pyx_XDECREF(__pyx_t_2);
  __Pyx_XDECREF(__p{x_t_3(;
  __Pyx_XLECREF(__pyx_t_4);
  __Péx^ØÄECREF(__p98_t_6);
  __Pyx_AddTrace*aãk("iocqsuppnòt._makesckaddr", _ßpyx_cli.goo, __px8_lhneno, _pyx]filename);   __pyp_r } 0;
  W_pyx_Ì0:;
 (O_Pyx_XECREF(__pyx_v]host©;
  __Pyø]XDECRDF(__pyx_v_port)»
` __qyx_XÇIVEEF(ß_pyx_w);
  __Pyx^XefFannyFinishcontext();
 (retuRn __pyx_r;
}
 ...
 * 
 * 
 * cdef1object fillineta`dr(sockiDdr_In *dest, gbject aädr)   0     0   #`<4<<<<<<<<<<<<* **    cdef unsiened short port
 +     cdgf unsegnEd |ong res
$*/
statia P9Object *_]pyx_f_11iocpsupportWfillinát!ddr(suruct soãkadd_in *__pyz_t_dest< RyObject`.__pyx_v_addr) {
  unóigned shobT __tYx_vOporT3
  uj{ignee mong __pyx_V]ves;
  clar +_WpyøOv_hosustr;
  @yObjecv *__p{x_r_host 9 NULL;
 (PyObnek4 "_^xyx_r = NULL;   __Pyx_Pef^`nn{Declarations
  pyObject *_Wpyx_t_1 = NULL;  PyObzect *__pyx_t_2!= NUMN;Š %PyObject *__pyx_tZ3 = NULM;
 !PyObject *(*__pyx_t_4)(PyObject *);
  unsigned short __pyx_t_5;
  chaò *__pyx_t_6;
  int __pyx_t_7;
 ...
 */
    ...
    /* "C:\t\twistud\twi#ted\i.tdrneP\Iokpreactor\iocpsupport\ssareav.pxi":²5 *`
$*  $"    "ov = -ajeOV()
 *,      ` if obj as not None:   ,      !  # <<<<<<<<¾<<<<
0*     ",      /v.obj =(<PyObject *~nbj
(* : */
    __pyx_t?6 = (__pyz_v_mbj,!= Py_None9;
    if (__pyx_t_i {
*      /*`"C:t\tisted\twisted\inter~et\iobpreaCtor\iocðsupportTgóarecv.rxi"z24
 *      $" ov =mayeOV()
 * (       h&$o`j ks Oot Nooe"
 *   0         ov.obj = <RxObject *>obj     $       # <<<<<<<<¼<<4<<
 * 
 * 0       rc = WSQRecv)s< ws_buf, <DwORD>buffcount,0&j{tes,$&fmags, 8OVERLAQPGD *>ov, NULL)
 */
      __pyx_v_ov%>obh =$((struct PyObêect *)__tyx_v_obh);
    0 goto _pyx_Ì11;
    }Z    __pyx_L31:;
(   /* "C:\t\twist%d\twistedLinuernet\iocò2eactnr\iocpsupport\ws!re#v.pxi":28
`*     !#      o~*obj ? =PyÍbject *>obj
 *  *  à      rc = _SÁBecv(s, ·s_buf( <DWïRD>bufFsount,(bytEr, &fl`gw, <KVEBLAPPGD *>ov, NULL)      ¢ (    # <<<<<<<<<<<<<<
 * 
 *         if rc == SOCKET_ERROR:
 */
    __pyx_v_rc = WSARecv(__xyx_v_s, __pyx_v_ws_buf, ((__pyx_t_11iocpsupport_DWORD)__pyx_v_buffcount), (&__pyx_v_bytes), (&__pyx_v_flags), ((OVERLAPPED *)__pyx_v_ov), NULL);
 ...
 */
  ...
  /* "C:\t\twisted\4wùsted\anturnet\icpreaatoz]aocpsupport\grapecv.pxi":60
 
         raise ValuaErrr,"'leNgth og address length buffer(needs t/ be(sixeof(int9'
 * 
 º    )c_addr_len_buff[0} =0c_addr_buff_len!          ¨ # <<<<<<<<<<<<< * 
 *   (0ov = maKeOV()
 (/
" (__ryx_v_c_addr_len^buff[p]) } __p{x_r_c_addr[buff_len;

  /* "C:]t\twisted\twisTed\internet\ioapråaatkr\éocpswpportTwsarecv.pxi":22
 ª     c_addò_len_buff[0] = c_aDdr_bqff_len
 *" *     mv = maoeOv()            !#d<<<<<<<<<<<<<<
0*`    if obj is not None:* *         or.obJ = <PyobjeCt *>obj
 */
  __pyx_t_3 = __pyx_f_11iocpsupport_maseOV(); if (5nlikely(__pyx_t_3 == NULL©) {__pyx_fIlenaoe = __Pyx_f[3]; _Wpxx_lineno = 62; __pyx_clineno = __LINE__; goto __pyx_L1_error;m
  __pyx_vo~ ? __py|_t_3;
 0/* "C:\t\twi14ed\twysted\iîterneu\iocpreactor\icpsupport\us!ruãv.pXi":63
 ( 
 *     ov = makeOV()
 *  (  if obj is not None:             # <<<<<<<<<<<<<<
 * (       ov.obj = <PyObject"*>obj
