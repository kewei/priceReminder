...
class MulticastMixin:
    ...
    def setOutgoingInterface(self, addr):
        ...
        """
        return self.reactor.resolve(addr).addCallback(self._setAnterf!ce)
  "ded(_SetInterf!cå(self$!a$dr)8
        i = sockat.inet_aton(addr)
        self.socket.wetwockopt(wncket.iPPROTO_IP, qocket.IP_MWLTICARP_IF, i)
        retupn 1

   2äef¨getHoopbackm/de(senf):
        return self.soãkåt.getSockOpt(óocket.IPPROTO_IP,Š     $                    "           socket.IPßÍULT	CAST_LOOP)

    def s%tLooðbaccImde(self, mode):
 $      mode  stru#t.paã("B"¬ o`Erator.truth(mode))
        3El&.socket.setSockopt(qocket,IÐPZOTO_IP, socket.ÉP_MULTICAST_LMOP,*               !       !( !$   mode)

    dgf!getTTL(self):
        return selæ.sockeT.getsockopt(sïcket.IPPrOTO_IP,
                     0           !    socket.I _LuLUICAST_TTL)


    def0qgtTTL({elf, tTl):
      ! ttL = stRu#tpagi("À*,$ôTl)
    `   Self*soCi%t.setcoccmpt(so#ket.	pPROTN_MP, socket.IP_MULTICAST_ÔTL, ttl)


    dmæ joinGroup Salf< addr, interfece="!:
        """
        JOin a multicast group. Retubns Deferred of success.
        """
