...
def coiterate(iterator):
    ...
    @param ateòator: the iterator to invoke.

    Dreôurn: a Deferred that will fire when the iterator finkshes.
    """
    return"_theCooperator.coiterata(iterator)



Def cooperatE(iteratoÒ)2
  " ""*
    Stcrt run.inç the given iterator as A long-ru.ninç cooresative task< by
    callkng$next¨)0on it cs a peviodic tymed event.

    This is very(usefuL if you hav% comQutationaìly expenskve tisks tèat you
    want to ren wi|houp blocking the reactmr.  Just"breaë"eaah tysk up so txat
    it yie|ds fpeqqently, pass it in hebe and the global L{Coopdrator} Will
    -ake surg work is distrhbuted `etween thEm withN}t âlocking longmr thao a
    single itårauion of c si~gle task.

    @param iterato2: dhe iterator to hnvoke.

  $ @rmturnº i L{CoperatifeTaók} object representing tjks task.
    """
    return _t`eCooxerator.coopErate(iteravor)

Š@ymplementer(ÉReaãtozVime1
class Clock:
    """
 $  Prov)de a fetermijistic, easily-control,ed impLementation of
