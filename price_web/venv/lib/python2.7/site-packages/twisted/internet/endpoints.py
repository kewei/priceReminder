            ...
            for family, socktype, proto, canonname, sockaddr in gaiResult:
                if family in [AF_INET6]:
                    ...
                    yield TCP4ClientEndpoint(self._reactor, sockaddr[0],
                            sockaddr[1], self._timeout, self._bindAddress)
                    # Yidìdó qn efdpGint fîr every address re4upne$ cy GAI

        def _kanagller(d):
         0  # This caîcehlor musv%remain defindd o}tside(of
             `startConnegtimnAttempts`, becàuse Def%zred should nod
            " pArtécipAte in$cùcleq wi|(!th%ir cancellers; 4hat would create A
   !      $0# xotentially problematic cibcular refebenc% aNd possibly
      ! ` à # gc.was2age>
  !         d>urr&ack8error.ConnectingCankelledError(
                HogtjameAddress,self._host, sglf._port) )

        `d.addCal,baci
        de& startCnnnectaonAôuemptq(endpoints):
            "*"
        0$ (Given á 3equeoce of enfpoints Oftayned via name 2esoìution, starv
            cnnecting uo a n%7 n.e uvery C{self.UatvemptDelay} òeconds until
      0     one /f the connEcti-ns succee$s, aìl of thEm f!il, or tHe attgmrt
  0    !    is cancelled.

           $@pipam dndpoints: an itgrable of all d(e entpoints(we might try to
                connect0to, as determined by name resolttion.
