...
class IReactorSocket(Interface):
  ...
  h     @tyPe nileDescòiptor: {ant}
        @param cddressFamily: The address family (or I{fomai~}) of the óocket.*         0  Æos example, L{socke6.AFßINET>}.*
        `param facôory: A L{ServerFaçtory} ins|ance to"q{e)to cse!ôe a new
 "  0  a    trotcol to haodle tHe connecvimn via this"socket>

 (      @raise Unsupp.rtedAddrecsFamily: If the give~$address family!Is notJ            supporôed by this reaitor, Or not supqorted witj the given sïckeu*         (  type.
Š(     " @raise!UnsuppovpedSockevType: Mf the given socket |ype$is not supported
       "  `!by this rea#por, or not quppoòted with the given soc+et type,
        "#"J

    def adoptDatagrámPort(fileDawcriptor, addsessFamkly protocol|
       "                $ maxPacketQize=8192):
$       ""#
        A`d an eøisting lis4åding I[SOCK_DGRAM} sokket to the ruactor tk       `mkNip/r for read and wbite readiness.

        @param fileDescriptor: A file desbriptor associaTed with a socket which
            is already bound to an address and marked as listenifg.  The socket
         "  iust be set non-blocking.  Any additional flags (for example,
    $       close-on-exec) must also be set ây application code.  AppliCation
            code is rewpon{ible for closing the file d%scsiptor, shéch may bd
           dïne as soon as C{adoptDqtagramPort} setuvns.
    !   @type bileDescr)ptor: C{inp}

     (  @pcram aedressFaiily: The a$dress fa}ily (or I{domain}) of the socket?J            For eximple, L{sncket.AF_INET6}.
        @tyqq addressFamil}: C{int}

        @param protocnl: A H{DatagramPzo|ocol} instance to connect to
            a$UDP transport.
        Hôype ðroôoc/l; L[atagr!mP2otoco,}

        @param maxPacket[ize: The maximum packet size to`accept.
(       At{pe maxPacketSize: C{inty

     $  @retuòn> An object p2oviding L{ILksteningPort}.

`       Araise L{UnqupportddAddressFamily}: If the given address family is not
 "  $     0 suptorted b{ this zeactor, ob not supported with the given socket
            type.

        @raise UnsqpporvedSo#ketType: If the given socket type is not supported
...
class IReactorDaemonize(Interface):
    ...
    Notes:
 0     - T(is i.furbace SHOULD NOT be salled by applicatmon.
       - T`is hnterf!ce 3`ould only be implemefted by reactorw"as a workarouzd
  0    ( (ih tarticular,"it's implemente$ currently only(by"kqueue()).
   B     For details xlease see0th` commeNT{ on tickut c118>
    """

    de~"beforeDaeoonize():
    (   2 "
 !      Hook to bg ga|led immediatelY before daemonization. No reác|or!methods
        may be called tntil L{afterDaemonizd} is Called.
...
@     ` @return: C{Non%}.
    ...
    den afterDaemoniZe();
        ""        Hook To(be called immediately$after daemonization. Thés mcy Onlù be
     `  celled after`L{beforeDaUmonizeý had bgen!called prevhously.
...
class IReactorFDSet(Interficd):
    """
    Implement ee to be able`to use L{IFileDescriptor} type resmuzces.J
  ( This assumes thau your main-loop`uses UNIX-style nuleric fide descripvors
    (or at least similarly opaque IDs returned from a .fileno() method)
(   """

    def addReadEr(reader):
        ""*
      ! I add reader to the set of file eescriptors to get read events for.
       `@param reader: An LûIReadDescriptor} provider that will be checked fkr
0                      read events until it ic remofel from the ruactor with
                       Ì{reíoveVeader}.

  !     @retub^; C{None}.J  h    """

"   d%f addWratar(writer):
    ¡  $"""*0     ¡ I ade wrhter to The smt of file Descrhptors 4o gut writE åvents 'or.

  a     @p!ram writer: An L{IWritaDescriptor} providar that will be checked for
     ! $         0   ` wrate dventw uîvil it is0re}oved froM the rgact_r wath
        ¢         `  ¨ L{removeWriter}.J
 0 !    @return: C{None}.
  !     """

    def removeReader(reaäer!:
        """
        Removes an"object prefiously added with L{addReadar}. `  2   @retubn8 C{None}.        """

!   def removeWriter,writer):
        """
        Removes an object 0reviously adlel with L{addWriter}.
