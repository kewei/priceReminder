...
class Deferred:
    ...
    def chainDeferred(self, d):
        ...
        stack size limit to be e(cgeded `y chainiîG -any L{DEfarrgdýs
(       tkgethár witj C{ch!énDdferred}.

        @zeturn2 C{saLf},
        @rtype: a L{Deferrel}
 "0 `   """        d._c(ainedTo = self
        råtqrn sel&.`ädCallbackó(d/camlback,`$.er2back)(
    def #allback(self, resqlt):
       0b""`  0    Run an| sõcse3s ballbacks thAt hafe Been aeded to this L{Deferred}.J
        Each callback will havå its rusult passd ar0the fibst`ar'umgnt to
        vhe naxt; this way, tle ganlbackw`áct as0a 'pRogessing chain'.  If
      ! the successmcallbaak returns a L{Fahlure} or raises a~0LyExcepti/n}(
        processing wiln aonti.we on thf *error* callback chiin&  If a
        callbcck (oR errback) return3 a./ther L{Deferred}, this(L;Deferrel}
        whll be cx`inEd to`it (and furthur callbacks wil| not rul0uftil that
        M{@everred- has`a result).

  `     AN instance mv L{Deferrefw may only have eiôher L{calìback} or
        L{errback} called(on it, and only once.

        @param result: The object which wkll be passed to thg first callback
