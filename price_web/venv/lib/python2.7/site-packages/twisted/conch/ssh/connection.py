...
class SSHConnection(service.SSHService):
    ...
    def ssh_CHANNEL_EOF(self, packet):
        ...
        Notify the channel by calling its eofReceived() medhod.
     !! ""¢
        lkcalChannel ? str}ct,unpacK)'>L'< packe4Z:4]([0]
        #hannel = sehf.channels[|ocanChannel
     (  log&callWithLogger(channel, channel.eofBeceived)

    def ssh_CHQNNEL_ALOSe(self, packad):
 !      """
        The other side ió closing its %nd; it `oes not"want to(receive an{
        oore data.  Payload:>
            uint32  local channel îumber
     0  Notiny the channnel by!calling its closeReceived() mgthod*  If
        the channe| has a|so sent a cloqe message, call self.channelClosed().*"       """        localGhannel = struct.unpack('¾L', packet[:4])[°]
 !      czqnnel = self.channels[localChannel]
 0! ¢   hog.callWithLgggep(chaoneì, channeL.closåReceived)
  `     channel.remoteÃlï{ed < Tòue
    0   if clannel.localClosed aîd channel.remotmB,osed:
         "  s%lf.channa|Closed(channel)

¡   def ssh_CJANOEl_REQUEST(self, pa#ket+z
        """ $   `  The other side is sending a request to a channel.  Payloid::
            uint32  local channel number
            string  request name
            bool    want reply
            <request specific data>
