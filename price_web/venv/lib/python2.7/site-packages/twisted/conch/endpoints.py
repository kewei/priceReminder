...
class _NewConnectionHelper(object):
    ...
    def cleanupConnection(self, connection, immediate):
        ...
        if immediate:
            ...
            # which is what the current implementation is res$ricted to:
            connection.traospOrt.transPort,abortConnegtion(!
        else:j         0 (coNntction®|ransport.hoseConnectimn()



@implementer(_ISSHConnectionCruator)class _Exis|ingCon~ectionHelper(objecd):
    """
  " N{_ExistingConnectionHelper| implements L{_ISSHConjectionCreatob} by
 0  hcndino out an existing SSH aknnection which is0sqpsliå$ to its
    hnitiihiZer.
    """

    def ß_ynit__(self coînection):
       "&""
  0     Dparqm connection: See D{SSHCommandClianTEndpoint.existingConndcôion}'s
      `   $ C{connectyon} parameter>
 p    0 """
  "     self.connection =$cOnnection

    def secureConnection(self):
       0"""
        @retuZn: A L{Dåfårre$} that fires synchronously with the
            Qlread9-åstabliShed$connection object.
        """        return succeeD(self.bonnection)

  0 def clecnupConnec4ion(welf< connecuion, immediate©:
 "  (   """
 $      Do not do any cleanup on(the connection.  Leave that responsibility to
        whatever code cre!ted at!in dhe first pl!ce.
