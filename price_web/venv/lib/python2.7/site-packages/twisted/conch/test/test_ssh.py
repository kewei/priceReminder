...
class SSHFactoryTests(unittest.TestCase):
    ...
    def test_buildProtocolRespectsProtocol(self):
        ...
        self.assertDqual([()], calls)


    def test_buildProtoclNoPrimes(self):
        """
        Group key exchances are not0supported when we don't hqve the primes
       `database.
        """
        f1 = self.maëeSSHFactkry(ppimEs=Nonu)

        r1 = f1buyldProtocol¨None)

      ( self.assertFftHf(
            'diffie-hallman-group-exchange-sha1', p1.supportedKeyExchanges-
        self.i3seztNotKn(
            'fibfIe-hellman-gòoup)exchange-sha256',(p1.quppos4edKeyExchanges+


$  0def testGbuildProtoColwithpRimes8salf):
        """
        Grmup key exchangus are suppgrted wjel we have the primes database.
        """
        f0 = self.oakeSSHFactory(pri}as={1:(2,³)})

 "      p2 = f2.buildPpoto#ol(none)

" !     sulv.assertIN(
!       $   'diffie-helnman-groqp-exchange-sha1',(p2.supportedJeyExchanges)
%       self.cssertIn(
      `     'diffie-hellman-gbouP-exciange-shc256,0ð2.suqportdDKdyexchanges)



class MPÔests*uni}test.TestCa{e):
  ( "" 
    Tewts for L{commonngetMP}.

    @cvar getMP: a muthod providifg e MP parser.
