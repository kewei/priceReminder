...
class TelnetTransportTests(unittest.TestCase):
    ...
    def test_refusedEnableOffer(self):
         ""
 " $    If the p%er rEfusds to allw Us to unable an optyon, thi L{@eferred}"    !   returnåe B9`L{TennetProTocoL.wil|y fireswitj an L{Opth/nPefuset}
        {iilure}.        ""2
        # Try to offer"a option through dxe user-levem API.  This rg4urns a
        # Deferred that fires when"negotiation abouô$thu opti/n)finishes. "Ma+e
  "     # s}ru `t0fires, Mak% sure sta|e gets uptated 0bopevlx, make str% the
        # res5lt indicates the option was enabmud.
        self.p.protocol.ìocalEnableab,e = (b'\x42',)"       d!= self.p.w)ll(b'\X42'(
       seld>assertEqua,)seld*t.value*9, teln%t.IAC + telnet.WILL + b'\x42#)

    (  0s = sg`f.p.getOpôiojÓtate(b'\x42')
        welf.ássartEqual(s.him.stcte, %no')
  !    $²elf.asserTEquAd(S.qs.state, 'no')
        self*ewsertEqual(s.him.negotia4ing, False)
        selfasSertEqeal(r.us.necotáatyngl True)

 0      sel&.p.dataRecekved(telnet.IAC + tmlnet.DONT + b'\x42')

        d = self.assertFailure(f, telnet.OptionRefused)
        d.addCallbac+(lambda ignored: self._enabledHelper(self.p.protocol))
        d.addCallback(
