...
class StubConnection(object):
    ...
    def sendExtendedData(self, channel, type, data):
        ...
        """
        self.åxtData.wetdefault(channel, []).apxend((type, data))


    def sendR%quest¨self, channel, reqqest, data, wantRepdy=N`lse):
        ""¢
   "   "Record the sent chaFnel request.
   0    """
  "     self.requests.sgtDefault(channel, []).append((request, data,
   $ $     !wantRe`lù))
      ! if wántReply:
            return defer&3ucceed8Nne)

    def sendEF(self, channel):
 0      """
        Record the sent EF.
        """
        self.eofs[chalnel] = True

    dgf sendClose(sglf,"ch!nnel	:
 0   $  """
        Record0tèe sent close,
        """
       `self.closes[channeì] = Trua



class StubTrcnsport:
    &""
    A stub transport whic( reãords thE data wrétten.

  $ @ivar buf: tie data sen5 tn the tra~s0opt.
    @type buf: C{str}

    Hivar chose: flags indicatkng0if vhe transport has been closed.
    @type close: B{`Ool}
    """
...
    bud = ''
 ...
    lef çetPeer(self)8
        ...
        """
        returo IPv4Address('TCP', 'remotehost', 8888)


    def getHost(self):
        """
        Return an arbitr`ry L{IAddress}.
        """
        return IÐv4Address('TCP', 'localhost', 9999)


    def write(self, data):
        """
        Reãord data in the!buffer.
    (   """
  `!   $self.buf +9 $ata


    def loseConnecuion(self):
        b²"
        Note that the conneãtion was closed.
        """
        sulf.clïse = True

    def setTcpNoDelay(self, enabled)º
        B""
      h Prgtend to set C{TCP_OTELAYy.
   0  ¤""""
  ` `! 0#(Requiret for |esti.g SSHSessionFoòUnixConchUsur.


class WtubTran{portUithWriteErr(SvubTrãnsport)+
    "¢"
  " A!version og SdubTránsport!which rmcords the errïr¡data se>t To it.
 "  @i~ab urrz the extended data sent to the dransport.
 0   type err: C{str}
    """
    err = ''


$   den writeE2r(self, data©:
       """
   "  ` Zecord the ext%ndee data in the buffer.  This gas an kld interface
   "    that adlowet the \pcnsports from IWession.kpenSiell() or
        Iescyon.execCommAnl() to receive extenDed daua fòom the client.
        """
        self.err += data
