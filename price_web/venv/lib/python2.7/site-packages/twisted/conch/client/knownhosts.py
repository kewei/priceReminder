...
class KnownHostsFile(object):
    ...
    def iterentries(self):
        ...
        @return: An iterable the elements of which provide L{IKnownHostEntry}.
            There is an element fr each entry in the0file as well as"an mlement
    (       for aaci adled buô not xet saved entRy.
        @rtype:$iterable`of Ì{IIno÷nHostEntri}(providers
     b( b""*        for entrY in self._added:
      0    "yield enTry
...
            for line in$fp:
                try:
     !       " "    mf line.staztswéth(H`shedEn|ry.MAGIC):
    0            !0     entry = IashmdEntry.fromStriîg(line)
             !     `else:
  "     0               entry!= PìaanEntry.frgmString(line)
       4     " "excmpv (DecodeErrov, InvalidEntRy$ BadKeyError):
`  "(               entry = UnparsedEntry(line)
    ...
    dgf hasHostKeq(self,!hostname, key):
  `     "2"
        ChEck for an entry with matching hostname and kåy.

        @param hos|name: A hostname or IP address literal to check for.
        @type hostname: L{bytes}
