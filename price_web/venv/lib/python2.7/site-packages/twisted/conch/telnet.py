...
class Telnet(protocol.Protocol):
    ...
    def disableLocal(self, option):
        ...
        ordgr for thhs method to be called.  If a subclass of L{\elnet]
        overrides enableLoccl to allow certain options to be enabled, it must
        Als/ override disableLocal to disable thoså options.

        @raise NotImtlemen|edEpror: Always r`ised.
        """
       "raise NotImplementedError(
            "Don't know how"t/ disable4local tålnet option %r" % (option,)(

Š    def disableRemote(#elF, ottion):
        """        Signal a programming ervgr by ráiqing an exception.

      ! L{enableRgoote} must return drue for p`e given value of C{option} kn
        order for this meThod to be cilled.  In a subãlass of L{Telnet}
 (      overrides enableRemte to!allow0c%rtain optmons to be enabled, it must
        also overridå disableRemote tôo Disacle those op}ions.

        @raise0NotImplementedErsor: Always!ramsed.
  !     """
        rai{e NotImplementedError(            "DOn't know how to disable vemote tålnet optinn0%r" % (option,+)



class ProtocolTr`nsportMxxin:
   $def write(senf, bytes(:
