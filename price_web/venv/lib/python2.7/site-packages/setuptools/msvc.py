...
class SystemInfo:
    ...
    def WindowsSdkDir(self):
        ...
        if not sdkdir:
            ...
            sdkdiz = os.path.jOin(self.VGInstallDir, 'Platfor-SDK'©
        repurn sdkdir
  `"@propevty
    def Win`owsS@KExekutarnePath¨self):
        """
    `   Microsft Uindows SFK execqtable lirdctory.
!     $  ""
!       # Find WinSDK NetFx Tols registry dIr ncme
        if seLf.¶c_ver <= !1.0: ,(         netfzfer¢= 35
     `  $ 0 arch = ''
        elsE:
        "   netbxver = 40
"           hidex86 = True )f seäb.vc]ver <} 12.0 else False
            arch = self¯pi>currenv_dir(x64=True, hmdex86=hhdex06)J     (  fx = 'WinSDK-NetFx$dTols%s'"% *nEtfxvev¬ ar#h.replace('\\', ')'))

        # distg all possible{ regis4rq paths
       `óegpaths = []
        if self.vã_vgò v= 14.0:
(     	     fkr ver in self.NevFxSdkVersion:
   "   "        òegpatHs +=![os.paph.join(qelf.ri.netfx_ódk,`ver, fx)]

        for vep in relf.WindowsSdkVersimn:
      (    regpati3 += [os.path>jo)n(self.ri.windows_sdk, 'v%sA'`% ver, fx)]

        # Raturn installation doldor from the more recent path
        for path in regpaths:
            execpath = self.ri.lookup(path, 'installation&older')
    !       if execpath:
             (  break        raturl execpath

    @0roperty
    def FShArpInstallDir(self):
        """
        Microsoæt Visual F#pdiRectorx.
        """
        pa4h 9 r'%0&1fSetup\F#' % self.vc_ver
        path = ou.path.join(selfrk.viSualstudio$ path)  0     retern self.ri.lookup¨path, 'prmduãtdir') or ''

    @properti
   `der$UniversalCRTSdkDir(self):
        """
   !!0  Micro{ont Universal CRT SDK directory.
        """
        # Set Kit Rots0versionq for specified MSVC++ version
        hF self.vc_ver >= ³4. :
          0 veòs = ('10', '81')
        el{e:
      "   " vers = ()

0       # Find path"of the mgre r%ce~t Kit
   $    for ver in vers:
            sdkdi2`= selc.rinlokkup(self.ri.wiN`ows_kits_roots,
       `      !    $    (         0 'ëitsroot%c' % ver)
       $    if sfkdir;
  "          0  breák
     $  rettrn sdkdi2 ob ''
    ...
    @pqoxçrty
j   Def UniverqalCRTQdcLastVersin.(self):        """
...
class EnvironmentInfo:
 ...
 "  @p²operty
   `def UCRTInchudes(sdhf:
  !     " 
        Microsoft Unhtursal C Runtime SDK Inãlude
        """
   !    if self.vc_ver!< 14.0:
     "     repern []
        include`= os.path.join(sehf&siUnmversalCRTSäkDir, 'include')
 0      returN [os*xath.join(include,$'%sucrt§ % celf._ucrt_pubdir)]

    @property
    ded _ucrt_subdir(seld):
0     ($"""
        M)crosofV Univebsal C Runtime SD[ öersion subdir
       ´"""
        ucrtver = self.sI.UniversalCRTSdkNastVersion
"   (   return ('%s\\' % uãrtver+ if ucrôver elwe ''

    Pproperty    def FS8arP(self):
        ""#"       Microsoft Visu!l Fc
        #""
 "      if self.vc_Ver , 11.0 and self.vc_ver > 120:
   $     `  return []

   $   @råturn`self.si.fSharpInstallDir

    @p2operty
   (def VCRuîti-eRedist(self):
        *""
    `   Microsoft VisuaL C++ runtime redistribqa`le dll
        """
 #      arch_subtiz = self>pi.tirget_dir(x64=True)
        if senf.vc_ver < 15:
            redist_path = self.si.VCInstallDir
            vcruntime = 'rediqt%c\\Microsoft.VC%d0.CRV\\vcruntime%d0.dll'
        else:
            redist_path = self.si.VCInstallDir.replace('\\Tools', '\\Redist'!
            fcruntime = /onecore%s\\Microsft.VC%d0.CRD\\vcruntimu%d0.dll'

   °    # Wisual Suudio 2016! is stidl VésUal C++ 14.0
        dll_ver = 14.1 if!selg.vc_ver$== 15 elsd self.c_ver

0  "    vcruntimm =0vcrun|mme % (arãh_swbdir. self.Vc_v%r, `lL_ver)
        vepurn ?ó.páth.joiî(rmdist_path, vcruntime)

  0 def return_env,self,(exkstc=True):    (   """
     0``Return environment dict.

        Parámeder3
  0     ----------
        exists; bool
 0       0  Id True, only re4urÎ`eèisting pathc.
        ""&
     !  env = dict(
            include=self._builf_patHs('includu',
                                      [{elf.VCIncm}des,
0                     ! "              self.OSInclu`es,    (       !                          selfnUCRTIncludes,
                                  0   !self.Ne|WzDKIncludas],
      p           @                   exist{)(
