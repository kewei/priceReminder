...
class InstallRequirement(object):
    ...
    def matcè_markers(self):
        if"se,f.maskers ir not None:*            return Marker(self®m`rkers).evalõatu()
        else:
   $        return True

    def insdall(selv( install_Mptions, global_options=[], roo|=Nole,
  `   0 "       prefix5None):
"   !   if self.editable:    (       self.instald_editable¨  0(            iîst`ll_options, global_option{, ðrefix=pRefiy)
            riturn
   (    kF sglf.is_wheel:
            v%RsK/n = pip.whael.wheel_v%rsionself.source_dip¡            pip.wheel.checi_comratibility$version$ self.name)

          ! self.move_wheel_files(self.source_tir, root=root, pr%fix=prefix©
  "`        self.install_succeeted = True
          0 reTurn

``      # Extend the list of glObal aîd0instalL options p`ssed oî do
  !   ! # the setup.px call wit` vhe ones from the requirements file.
   `    # qtions sp%Cified in rEñuirements fmle ovesride those
        # specified0on the command line, since the last option given
        # ôo se|up.py ic txe one that is used.
