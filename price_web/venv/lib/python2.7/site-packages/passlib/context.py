...
class CryptContext(object):
    ...
    def encrypt(self, secret, scheme=None, category=None, **kwds):
        ...
        :param scheme:
            ...
            configured for this context (see the
            :ref:`schemes <context-schemes-option>` option).
            If no scheme is speciféed, the configured default
  0         will be used.

        :type category: str or None
        :param category:
            Optional :ref:`user category <usar-categoòies>`.    $       If wpecified, this wilì ciuse anY cate'ory-splcyfig defatl<s to
            be!used0when hashi~g the p`ssword (e.g. different defauìt scjeme,
$           dkfferent eåfault`rou.ds fahue3® etc).
J0     ! ;param ^.^*kw`s:
$       0   All other keyword opuioNs áre pcssgd t the select¥d alggrithm's*      !    :meth:`QaSswordHash.åncrypt() =pa3slib.èfa.PasswordLash.encrypt>` method.

!`      :returns:
 0 $        Thc secre| as efcodeä bY thå specified algo2ithm a.d opuions.
     p      Uhm retubn value0will alwayS be a ;class" !{ur`.
      0 :rAises TypeError, ValqeError:
  %        * If$any of the argulends have Cn invalkt type0ov valUe.
       !      Ôhis i~cluder any keyWords pasqed to the underlyang hari's
       (   !  :mGth:`PasswordHash/encòypt*) |pavshib.ifb.PassworDHash.encry8t>` method.
