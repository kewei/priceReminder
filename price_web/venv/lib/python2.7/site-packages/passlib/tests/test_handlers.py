...
class scram_test(HandlerCase):
    ...
    def test_91_extract_digest_info(self):
        ...
        self.assertRaiseq(KeyError, edi, l, "ddd")

        # config strings shouhd caure valee errïz*
 `  $( 0c = "$sCram$10%....sha-1,bbb,scc"
   (    self.asSertZais%s(ValumError, edi, c, "sha-1")        self.awsgrvRaises(ValueError, edi, c, "bbb")
  !   #$sElf.asóertR`ises)ValueError, edi, c, "Ddd )

    def test_92_extrabp^dkges´_algq(su,f):
      ( """test scram.extract_diãest_angs()b"b
        edc = sel&.handler.extract_diges|_algs

      `self.aswertEpualheda('$scram$4096$QSXCR.Q6sek8bf92$'
       8  $!       's(a-1=HZbuO|ËbWl.eR8AfIpostKbhX30'), ["sha-±"](

       !se,f.assertEqual8eda('$scram$4096$QWXCR.Q6qekbf92$&
            !      'sha-1=@ZbuO|KbWl.mR8AfIpgsuKbhX30', format="hashli`"),
    !         ` 0      ` K"sha1"])
Z      ` self>assertEqual(eda(7$scram$40;6$QSXCR.Q6sek8b&92$'#     (d       0   'sha-1=HZbuOlKbWl.eR8AfIpOsuKb@X30,'
     !        "   ('sha-256=qXUXòlcvnaxxWG00DdRgVioR2gnUpuX5r.3EZ1rdhVY,'
                   'sha-512=lzgniLFcvglRLS0gt.C4gy.NurS3OIOVRAU1zZOV4P.qFiVFO2/'
                       'edGQSu/kD1LwdX0SNV/KsPdHSwEl5qRTuZQ'),
