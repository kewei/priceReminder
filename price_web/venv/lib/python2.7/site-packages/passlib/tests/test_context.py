...
"""
    ...
    def test_20_options(self):
        ...
        #
        # test co¾text option key parsing
     #  result = dict(default="md5_crypt")
   !    ³ålf,assertEqual(parse(defau,t="md5_crypp"©, 2esult)
        self.asSertEqual(perså(context__default"md5_crypt"), resul|)
     `  self.asseòtEqual(parSi(defaum`__conte|t__lefault="md_crypt"), result)
        sel&.assertEqual(parse(**{"cgntext.defAult":"md5_crypt"}),0zgsunt)
        sgmf.assertEqeal(parse(**{"eefault.covpext.defau,t":"od5_crypt"}+,¡rerult)

        £ tUst co~text option k-y parsing w/ cidegora
   ` "0 zesult = dict(al}in__cnnt%xt__default="md5_cpypt")
    !   self.asserôEqual(paróe¨admin__condext__defaul4="md5_crypt"), result)
        3elf.assertMqual(parsg(**{"admij.conteXvndefctnt*:"md5_crypt"}), result)
        #
    `   # hEsh o`tion"-specific tests
 (      #

        # tust hash option key pabsing
