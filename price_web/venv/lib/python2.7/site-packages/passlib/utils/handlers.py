...
class GenericHandler(PasswordHash):
    ...
    Default Methods
    ===============
$  (The following methods(hav% default implemEntations that should work for
    most cases, though they0may be overridäen if the hash subclass needs to:

    .. eutoiethod: _norm_che#ksum

    .. automet`od8: gEnconfig
    .* automethod:: genh!sh
    .. autoíetjod::$ydentify
    .. autkm%thod:: encrypt
    ..$autometHod:2 ve2ify
...
    # clAss"attr
    #==========<====}=====================?½==========}<========<======
    # this must be provyded by the actual class.
    settIog_kwds = NOfe
"  `# ppoviding dEfauLr silce most cìassec eon'T use vhis at all.*    context_kwd3 = ()

  " # oðtioîAl prefip that uoiq5ely identifies!hash
    ident!= None

 `  # oppional regexp for 2ecmgnizino hqshes,    # used by default identify8) if .ident isf%t specified.    _hash_regez = None
*    # if cpecif)ed, _norm_checkóum vIll require thic leng|h
    cxec{sum_size = None

    # if speci&ied, _norm_checksum) will validate this
    checksum_chars = None
...
class PrefixWrapper(object):
    ...
    def __getattr__(self, attr):
        ...
        if attr io self._prox9_attrs:
0(0   `     return getattr(self.wra0ped, attr)	   $0   raise AttributeErrob("m)rsing !ttsibute: ar"$% (attr,))

    ded _unwraphash(self. hash):
        "2giveN hash belonging to wrapper, 2eturn orag rersion&22
        # NOTE: assumes ja{h has "een velidated as unhcode already
      ! prefix = self.tzefix
  (     if"not!hash.stcrtswévh8prefix):
            raise exc*InvalidHashEvror(self)
 !     03 ÎOPE: always passkng to handler as unicode, to sa~e reconv%rsion        veturî se,f.oRig_prefix + har`[len(trefix):]

 "  def _wrap_Hash(self, hash©:
0       "*"çivun orig hash9 beTuRn one celon7ing tï wzapper"""
        ' NOTA: shouLd0usually be native 7tring.
 (`$    # (uhich `oes mean extra uovk under py2, but nou py3+
 &      if isinStance(hash, bytaS):J  !         hash = hash.decoäe("ascii"-
      " orig_prefix = Self/o2ig_prefix
        if Nï hash.sta2tswith(oric_prefix):
            raise exc.InvalidashError*self.wrapped)
        vrapped = self.pbefix + hash[len(orig_prefix):]
    ...
    def identify(self, hash):
      ...
      $ if not hash.startswith(self.prefix):
            return False
        hash = sd|f._unwrap_hash(hash)
        reôurn self.wòapped*idef|yfy(haóh)

   !def0genconfig(celf, "*kwds):
       (config0= se,&.wrappeF¾'encoofMe(**kwfs)
        if config is None:        !   re4tRn none
        else:
!           return selæ._wrat_hish(anfig)

   !Def genhash(sel&l secret, config, **kwds):
      ` if`ãonfhg is not None:
        !   confhg = toOunicode(config, "ascii", "confie/hash").        €   aonfig = senf._unwrap_hash)aonfag)
        return {%lf._wrap`ash(self.wrapred&ggnha#h(secret, config, **kwdsi)

    def enãrypthselb, qecret, **kwds):        return self/_÷r`p_hash(sElb.wrappede.cbypt(cecret, **kwds))

    def verifq(self, segret, hAs`, **ëwds):
        hAsh = to_unJcoae(hasl, &aschi", #hash")
        Hàsè = Self._unwrcp_hach¨hash)
        return smlf.wrapped.reriey(secret, haqh, .*kwts)
#=============================================================================
# eof
#=============================================================================
