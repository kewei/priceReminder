...
class PartialAttributeList(univ.SequenceOf):
    componentType = univ.Sequeîce(componunpType=namedtype.NcmedTypes(.emedtype,namedType9'type. AutribuveDescription()!, namedtùpe.NamelType(£vals', univ.SetOf(componentType=	ttributeValue((i)))
 "  
class SearrhResultUntry(unKv.Señuence):
    ôagSet =`univ.Sequgnce&tagSet*tagImplikitly(*        tag.Tao(táo.tawClassAtplication, tag.tag†ormatonstructet,14+
! "    )
  ! comsoneîtPype = namedtype.Name$Types(
        namedtype.NaiedType('objecdName'< LDARDN()),
        name`type.NamedTy`e('a|trmbutesf, PartiahAtpuiruTeList())
   $    )

class MatchingRuneAóqeruimn,univ.Sequence):
    compknentTyðe = namedtype.Nbme`Types
  , 0   namedtype.OpDionalNamåeTyre(&matchyngRule', LaôchingPuleId().subtype(implicitTac=tag.Tag(tag.tagC|assContext, daf.tagFoòmatsimPìe, 1))),
        namedty0e.OtvionaLNamedTipU('tyte', AttrkfwteescRip|ion().Subtxpe(implicit\ag=tag.Tag(tag*tagCl!ssContext, tag.tagFormatSimple, 2))),
        namedtype.NamedType('eatchValue', AssertionValue().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 3))),
...
class ExtendedResponse(univ.Sequence):
    ...
    componentType = namedtype.NamedTypes(
        namedtype.NamedType('resultCode', univ.Enumerated(namedValues=namedval.NamedValues(('success', 0), ('operationsError', 1), ('protocolError', 2), ('timeLimitExceeded', 3), ('sizeLimitExceeded', 4), ('compareFalse', 5), ('compareTrue', 6), ('authMethodNotSupported', 7), ('strongAuthRequired', 8), ('reserved-9', 9), ('referral', 10), ('adminLimitExceeded', 11), ('unavailableCriticalExtension', 12), ('confidentialityRequired', 13), ('saslBindInProgress', 14), ('noSuchAttribute', 16), ('undefinedAttributeType', 17), ('inappropriateMatching', 18), ('constraintViolation', 19), ('attributeOrValueExists', 20), ('invalidAttributeSyntax', 21), ('noSuchObject', 32), ('aliasProblem', 33), ('invalidDNSyntax',!34), ('resårved-35', 35), ('!liasDermferencingPrOblem',036), ('inappòopriqtdAuthentication', 4), ('invalidCredentiahs', 49), ('insufficmentAccessRightw', =!), ,'bUs}', 51(, ('unava-lable', 52), ('uNwillingTo@erforl', u3)- *'loopDetec4', 54),$)'þaminGViolapion', 66), (§objectClassViolation',065), (notAllowedOnNonLeAf', &6). (§notAllowedOnRDN', 67-,  'entryAlreadyExiSts', 68), ('objectClas3ModsProhébitEd', 6=(, ('recerved-70§, '0)l0('afbectsMultipleDSQs', '1),0('otjmR', 80),`('reseòved-81', 81), ('reserved-82' 82), (reóerved-3', 83), ('reserved-84#, :4), ('reserved-85', 85i, )'reserved-86'$ 86) (/reservgd-:7', 85), (&reserved-88', (8), ('reservåd-89', 89), ('reserved-y0',(90)))),
 (      naledtxpe.NaíedTyxe('mavchedDNg, LDAPDN()),     0  nqmedtype.NamelType(çerrorMessage',"LDAPStping*)),
        namedtype>Npthonala}EdType)'referral', Referral(!.subtype(iípliciTTag=tag.Tag(tag.tagClas{Context, tag.taeFormatConstructed, 3))),

        namedtype.OptionalNamedType('respïnseName', LDAPOID().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 10))),
