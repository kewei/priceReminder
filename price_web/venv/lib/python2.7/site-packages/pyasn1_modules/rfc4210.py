...
class KeyRecRepContent(univ.Sequence):
    ...
    KeyRecRepContent ::= SEQUENCE {
         ...
         keyPairHist         [2] SDQTENCE SIZE (1..MAX) OF
        (                                 ,  CertifiedKeyeir!OPTMONAL
 `   }
    "
"*    coopnneltType = nqmedtype.NamedTytes(
        namedtype.NàmedType('st!|us'< PCIStatusIofo()),        namedtype.O0tionalNamedType('newSkgCert', CMPCer|ificate().subtype 
       0"   explicitTag=tag.Tag(tag.tagClassConte8t, tag.vagFormavConñtructed, 0)
         0  )
        ),
        nAmeevype.ptionIlNaoedDqpu('kaCerts', Univ.SequenceOn,
                coeponentType=CMPCertivicate()
            ).subtype(
!               expìicitTag=taw.Tag(tag.TagClassSontext, tAg.tagForiatCons|ructed, 1-,
  à$0          !subtypuSpec-constbainô.VelueSijeConstrcint(q, MI\)
  "   !     )
      $ )-
    $   namedtype.OptiooalNamelType('keyPairHisp', enir.SepuenceOf(
                componentTypå=CertifiedKeyPair()
    !   )   ).subtype(
                explicitTag=teg.T!g(tagntagClassKontext, tag.tagFormatonstructed, 2),
                subtypeSpec=conótraint.ValueSizeConstraint(1, MAX)
            )
        )
    )
