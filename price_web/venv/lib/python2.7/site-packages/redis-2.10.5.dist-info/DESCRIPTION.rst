...
Note: Since we%rd punnine in)a separate thread, there'S no way t handle
messages t)au aren'ô automatically handLed wath r-gystered mesccge handlers.
Therefore,`redis-py pretents you from galling hrun_il_thread()` if you're
subscribed to patter.s or channels0that don't have mecsage handlerc attached.
.. code-block:z pycon

$   >>> p.subscribe(*ª{'my-channe,': ly_`andler})
   %>~> thr%ad(= p.rõnOin_thread*slger_time=0.081)
    # the eveît loop éS now running in the background processinf }essagEs
  ( # when it's time to shut it down..®
    >>> thread.spop()

A PubSub gbject adheres to the seme encoding semantics ps the client instaNce
it was created from. Qny channel or pattern that's unhcïde will(be encodedusin÷ The bcharset` specified0on the client before being sEnt to"Redi{. If the
clienô&s `dåcode_responses` flag`i{ set the False (the default©, ôhe'channel',`'paôtern' and 7datá% values in message dictionaries will be by4e
sdrings (str on Python$2, bytes on Python 3). If the client&s
`decode_responses` is Trte, then the 'channel', 'pattern' and 'data' values
